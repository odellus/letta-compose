...
def create_karla_agent(
    ...
    # This tool provides str_replace, insert, delete, rename operations on memory blocks
    try:
    ...
    # Register karla tools with the agent
...
def get_or_create_agent(
    ...
    if agent_id:
        try:
            ...
            # Ensure the unified 'memory' tool is attached
            try:
                memory_tools = list(client.tools.list(name="memory"))
                if memory_tools:
                    memory_tool = memory_tools[0]
                    client.agents.tools.attach(agent_id=agent.id, tool_id=memory_tool.id)
                    logger.info("Attached memory tool to existing agent: %s", memory_tool.id)
            except Exception as e:
                logger.warning("Could not attach memory tool to existing agent: %s", e)
