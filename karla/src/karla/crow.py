"""Crow integration layer for tool registration and agent interaction."""
...
def register_tools_with_crow(
    client: Crow,
    agent_id: str,
    registry: ToolRegistry,
    requires_approval: bool = True,
) -> list[str]:
    """Register tools from a registry with a Crow agent.
    ...
    is how Crow hands control back to the client for execution.
...
class CrowAgent:
    """Wrapper around a Crow agent with client-side tool execution."""
    ...
    async def _iterate_stream(self, stream):
        """Iterate over a Crow stream, handling sync iteration in async context."""
        # The Crow SDK returns a sync iterator, wrap it for async
        for chunk in stream:
