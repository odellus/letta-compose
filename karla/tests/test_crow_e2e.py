"""Real Crow end-to-end tests for karla.

These tests:
1. Connect to a running Crow server
2. Create agents with karla tools
3. Send messages and execute tool calls client-side
4. Send results back to Crow and continue the loop

Based on the Lares pattern for client-side tool execution.

Run with: pytest tests/test_crow_e2e.py -v -s
"""
...
def send_message(client: Crow, agent_id: str, message: str) -> MessageResponse:
...
def send_tool_result(
    ...
    """Send tool execution result back to Crow."""
    response = client.agents.messages.create(
        agent_id=agent_id,
        messages=[{
            "type": "approval",
...
def crow_server_available() -> bool:
    """Check if Crow server is running."""
    try:
        client = Crow(base_url="http://localhost:8283", timeout=5)
...
pytestmark = pytest.mark.skipif(
    not crow_server_available(),
    reason="Crow server not available at localhost:8283"
)
...
class TestCrowConnection:
    """Test basic Crow connectivity."""

    def test_server_responds(self, crow_client):
        """Verify Crow server is responding."""
        tools = list(crow_client.tools.list(limit=1))
        assert tools is not None


class TestToolRegistration:
    """Test registering karla tools with Crow."""

    def test_register_tools(self, crow_client, karla_config, tool_registry):
        ...
        agent = crow_client.agents.create(
            ...
            embedding="crow/crow-embed",
            include_base_tools=False,
        )

        try:
            registered = register_tools_with_crow(crow_client, agent.id, tool_registry)
...
class TestToolExecutionLoop:
    ...
    @pytest.fixture
    def agent(self, crow_client, karla_config, tool_registry):
...
Always use tools to complete tasks.""",
        ...
        register_tools_with_crow(crow_client, agent.id, tool_registry)
    ...
    def test_read_file(self, crow_client, agent, tool_executor, temp_workspace):
    ...
    def test_write_file(self, crow_client, agent, tool_executor, temp_workspace):
    ...
    def test_bash_command(self, crow_client, agent, tool_executor, temp_workspace):
    ...
    def test_multi_tool_workflow(self, crow_client, agent, tool_executor, temp_workspace):
...
class TestErrorHandling:
    ...
    def test_read_nonexistent_file(self, crow_client, agent, tool_executor, temp_workspace):
