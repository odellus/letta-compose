...
@dataclass
class CrowConfig:
    config_path: str = os.getenv("MEMGPT_CONFIG_PATH") or os.path.join(CROW_DIR, "config")
    ...
    metadata_storage_path: str = CROW_DIR
    ...
    crow_version: str = crow.__version__
    ...
    @classmethod
    def load(cls, llm_config: Optional[LLMConfig] = None, embedding_config: Optional[EmbeddingConfig] = None) -> "CrowConfig":
        # avoid circular import
        from crow.utils import printd

        # from crow.migrate import VERSION_CUTOFF, config_is_compatible
        # if not config_is_compatible(allow_empty=True):
        #    error_message = " ".join(
        #        [
        #            f"\nYour current config file is incompatible with Crow versions later than {VERSION_CUTOFF}.",
        #            f"\nTo use Crow, you must either downgrade your Crow version (<= {VERSION_CUTOFF}) or regenerate your config using `crow configure`, or `crow migrate` if you would like to migrate old agents.",
        #        ]
        #    )
        ...
        if os.getenv("MEMGPT_CONFIG_PATH"):
            ...
            config_path = CrowConfig.config_path
        ...
        if os.path.exists(config_path):
            ...
            config_dict = {
                ...
                "crow_version": get_field(config, "version", "crow_version"),
            }
    ...
    def save(self):
        ...
        # set version
        set_field(config, "version", "crow_version", crow.__version__)
    ...
    def create_config_dir():
        ...
        for folder in folders:
            if not os.path.exists(os.path.join(CROW_DIR, folder)):
                os.makedirs(os.path.join(CROW_DIR, folder))
