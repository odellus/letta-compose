...
from crow import __version__, system
from crow.errors import CrowImageFetchError
from crow.schemas.enums import MessageRole
from crow.schemas.crow_message_content import Base64Image, ImageContent, ImageSourceType, TextContent
...
async def _fetch_image_from_url(url: str) -> tuple[bytes, str | None]:
    ...
    headers = {"User-Agent": f"Crow/{__version__}"}
    try:
        ...
        raise CrowImageFetchError(url=url, reason=str(e))
    except Exception as e:
        raise CrowImageFetchError(url=url, reason=f"Unexpected error: {e}")
...
async def _convert_message_create_to_message(
    ...
    for content in message_content:
        if isinstance(content, TextContent):
            ...
            if content.source.type == ImageSourceType.crow and not content.source.data:
                # TODO: hydrate crow image with data from db
                pass
