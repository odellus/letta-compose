...
from crow.functions.mcp_client.types import StdioServerConfig
from crow.schemas.embedding_config import EmbeddingConfig
from crow.schemas.enums import ProviderType, ToolRuleType
from crow.schemas.crow_message import ApprovalReturn, MessageReturnType
...
from crow.schemas.llm_batch_job import AgentStepState
from crow.schemas.llm_config import LLMConfig
from crow.schemas.message import ToolReturn
from crow.schemas.response_format import (
...
from crow.schemas.tool_rule import (
    ChildToolRule,
    ConditionalToolRule,
    ContinueToolRule,
    InitToolRule,
    ...
    for content in message_content:
        if isinstance(content, MessageContent):
            if content.type == MessageContentType.image:
                assert content.source.type == ImageSourceType.crow, f"Invalid image source type: {content.source.type}"
...
def deserialize_message_content(data: Optional[List[Dict]]) -> List[MessageContent]:
    ...
    for item in data:
        ...
        if content_type == MessageContentType.text:
            ...
            assert item["source"]["type"] == ImageSourceType.crow, f"Invalid image source type: {item['source']['type']}"
