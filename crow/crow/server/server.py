...
import crow.constants as constants
import crow.server.utils as server_utils
import crow.system as system
from crow.config import CrowConfig
from crow.constants import CROW_TOOL_EXECUTION_DIR
from crow.data_sources.connectors import DataConnector, load_data
from crow.errors import HandleNotFoundError, CrowInvalidArgumentError, CrowMCPConnectionError, CrowMCPTimeoutError
from crow.functions.mcp_client.types import MCPServerType, MCPTool, MCPToolHealth, SSEServerConfig, StdioServerConfig
from crow.functions.schema_validator import validate_complete_json_schema
from crow.groups.helpers import load_multi_agent
from crow.helpers.datetime_helpers import get_utc_time
from crow.helpers.json_helpers import json_dumps, json_loads

# TODO use custom interface
from crow.interface import (
    AgentInterface,  # abstract
    CLIInterface,  # for printing to terminal
)
from crow.log import get_logger
from crow.orm.errors import NoResultFound
from crow.otel.tracing import log_event, trace_method
from crow.prompts.gpt_system import get_system_text
from crow.schemas.agent import AgentState, CreateAgent, UpdateAgent
from crow.schemas.block import Block, BlockUpdate, CreateBlock
from crow.schemas.embedding_config import EmbeddingConfig

# openai schemas
from crow.schemas.enums import AgentType, JobStatus, MessageStreamStatus, ProviderCategory, ProviderType, SandboxType, ToolSourceType
from crow.schemas.environment_variables import SandboxEnvironmentVariableCreate
from crow.schemas.group import GroupCreate, ManagerType, SleeptimeManager, VoiceSleeptimeManager
from crow.schemas.job import Job, JobUpdate
from crow.schemas.crow_message import LegacyCrowMessage, CrowMessage, MessageType, ToolReturnMessage
...
from crow.schemas.memory import ArchivalMemorySummary, Memory, RecallMemorySummary
from crow.schemas.message import Message, MessageCreate, MessageUpdate
from crow.schemas.passage import Passage
from crow.schemas.pip_requirement import PipRequirement
from crow.schemas.providers import (
    AnthropicProvider,
    AzureProvider,
    BedrockProvider,
    DeepSeekProvider,
...
from crow.schemas.sandbox_config import LocalSandboxConfig, SandboxConfigCreate
from crow.schemas.secret import Secret
...
from crow.server.rest_api.chat_completions_interface import ChatCompletionsStreamingInterface
from crow.server.rest_api.interface import StreamingServerInterface
from crow.server.rest_api.utils import sse_async_generator
from crow.services.agent_manager import AgentManager
from crow.services.agent_serialization_manager import AgentSerializationManager
from crow.services.archive_manager import ArchiveManager
from crow.services.block_manager import BlockManager
from crow.services.file_manager import FileManager
from crow.services.files_agents_manager import FileAgentManager
from crow.services.group_manager import GroupManager
from crow.services.helpers.tool_execution_helper import prepare_local_sandbox
from crow.services.identity_manager import IdentityManager
from crow.services.job_manager import JobManager
from crow.services.llm_batch_manager import LLMBatchManager
from crow.services.mcp.base_client import AsyncBaseMCPClient
from crow.services.mcp.sse_client import MCP_CONFIG_TOPLEVEL_KEY, AsyncSSEMCPClient
from crow.services.mcp.stdio_client import AsyncStdioMCPClient
from crow.services.mcp_manager import MCPManager
from crow.services.mcp_server_manager import MCPServerManager
from crow.services.message_manager import MessageManager
from crow.services.organization_manager import OrganizationManager
from crow.services.passage_manager import PassageManager
from crow.services.provider_manager import ProviderManager
from crow.services.run_manager import RunManager
from crow.services.sandbox_config_manager import SandboxConfigManager
from crow.services.source_manager import SourceManager
from crow.services.step_manager import StepManager
from crow.services.telemetry_manager import TelemetryManager
from crow.services.tool_executor.tool_execution_manager import ToolExecutionManager
from crow.services.tool_manager import ToolManager
from crow.services.user_manager import UserManager
from crow.settings import DatabaseChoice, model_settings, settings, tool_settings
from crow.streaming_interface import AgentChunkStreamingInterface
from crow.utils import get_friendly_error_msg, get_persona_text, make_key, safe_create_task

config = CrowConfig.load()
...
class SyncServer(object):
    ...
    def __init__(
        ...
        if settings.database_engine is DatabaseChoice.POSTGRES:
            ...
            config.recall_storage_uri = settings.crow_pg_uri_no_default
            config.archival_storage_type = "postgres"
            config.archival_storage_uri = settings.crow_pg_uri_no_default
        config.save()
        self.config = config
        ...
        # collect providers (always has Crow as a default)
        self._enabled_providers: List[Provider] = [CrowProvider(name="crow")]
    ...
    async def init_async(self, init_with_default_org_and_user: bool = True):
        ...
        if init_with_default_org_and_user:
            ...
            tool_dir = tool_settings.tool_exec_dir or CROW_TOOL_EXECUTION_DIR

            venv_dir = Path(tool_dir) / venv_name
            tool_path = Path(tool_dir)

            if tool_path.exists() and not tool_path.is_dir():
                logger.error(f"CROW_TOOL_SANDBOX_DIR exists but is not a directory: {tool_dir}")
            else:
                if not tool_path.exists():
                    logger.warning(f"CROW_TOOL_SANDBOX_DIR does not exist, creating now: {tool_dir}")
                    tool_path.mkdir(parents=True, exist_ok=True)

                if tool_settings.tool_exec_venv_name and not venv_dir.is_dir():
                    logger.warning(
                        f"Provided CROW_TOOL_SANDBOX_VENV_NAME is not a valid venv ({venv_dir}), one will be created for you during tool execution."
                    )
    ...
    async def init_mcp_clients(self):
        ...
        for server_name, server_config in mcp_server_configs.items():
            if server_config.type == MCPServerType.SSE:
                ...
                raise CrowInvalidArgumentError(f"Invalid MCP server config: {server_config}", argument_name="server_config")
        ...
        if request.llm_config is None:
            ...
            if request.model is None:
                if settings.default_llm_handle is None:
                    raise CrowInvalidArgumentError("Must specify either model or llm_config in request", argument_name="model")
                else:
                ...
                if isinstance(request.model, str):
                    ...
                    raise CrowInvalidArgumentError("Multiple models are not supported yet")
                else:
    ...
    async def insert_archival_memory_async(
        ...
        from crow.services.context_window_calculator.token_counter import create_token_counter
    ...
    async def get_agent_recall(
    ...
    ) -> Union[List[Message], List[CrowMessage]]:
        ...
        if not return_message_object:
            records = Message.to_crow_messages_from_list(
                messages=records,
    ...
    async def get_agent_recall_async(
        ...
        if not return_message_object:
            ...
            agent_state = await self.agent_manager.get_agent_by_id_async(agent_id=agent_id, actor=actor)
            text_is_assistant_message = agent_state.agent_type == AgentType.crow_v1_agent
    ...
    async def get_all_messages_recall_async(
        ...
        if not return_message_object:
            # NOTE: We are assuming all messages are coming from crow_v1_agent. This may lead to slightly incorrect assistant message handling.
            # text_is_assistant_message = agent_state.agent_type == AgentType.crow_v1_agent
    ...
    def get_server_config(self, include_defaults: bool = False) -> dict:
        ...
        if include_defaults:
            default_config = vars(CrowConfig())
    ...
    async def load_file_to_source(self, source_id: str, file_path: str, job_id: str, actor: User) -> Job:
        ...
        # try:
        from crow.data_sources.connectors import DirectoryConnector
    ...
    async def get_embedding_config_from_handle_async(
        ...
        except CrowInvalidArgumentError as e:
    ...
    def get_local_llm_configs(self):
        ...
        # NOTE: deprecated
        # try:
        #    llm_configs_dir = os.path.expanduser("~/.crow/llm_configs")
    ...
    def get_local_embedding_configs(self):
        ...
        # NOTE: deprecated
        # try:
        #    embedding_configs_dir = os.path.expanduser("~/.crow/embedding_configs")
    ...
    async def run_tool_from_source(
        ...
        from crow.services.tool_schema_generator import generate_schema_for_tool_creation, generate_schema_for_tool_update
        ...
        try:
            ...
            from crow.schemas.crow_message import ToolReturn as ToolReturnSchema
    ...
    async def get_mcp_servers(self) -> dict[str, Union[SSEServerConfig, StdioServerConfig]]:
        ...
        # Attempt to read from ~/.crow/mcp_config.json
        mcp_config_path = os.path.join(constants.CROW_DIR, constants.MCP_CONFIG_NAME)
    ...
    async def get_tools_from_mcp_server(self, mcp_server_name: str) -> List[MCPTool]:
        ...
        if mcp_server_name not in self.mcp_clients:
            raise CrowInvalidArgumentError(f"No client was created for MCP server: {mcp_server_name}", argument_name="mcp_server_name")
    ...
    async def add_mcp_server_to_config(
        ...
        try:
            ...
            raise CrowInvalidArgumentError(f"Failed to parse MCP config file {mcp_config_path}")
        ...
        try:
            ...
            raise CrowInvalidArgumentError(f"Failed to write MCP config file {mcp_config_path}")
    ...
    async def delete_mcp_server_from_config(self, server_name: str) -> dict[str, Union[SSEServerConfig, StdioServerConfig]]:
        ...
        if server_name not in current_mcp_servers:
            raise CrowInvalidArgumentError(f"Server name {server_name} not found in MCP config file", argument_name="server_name")
    ...
    async def send_message_to_agent(
        ...
        stream_steps: bool,
        stream_tokens: bool,
        # related to whether or not we return `CrowMessage`s or `Message`s
    ...
    ) -> Union[StreamingResponse, CrowResponse]:
        ...
        try:
            ...
            crow_agent = self.load_agent(agent_id=agent_id, actor=actor)

            # Disable token streaming if not OpenAI or Anthropic
            # TODO: cleanup this logic
            llm_config = crow_agent.agent_state.llm_config
            ...
            crow_agent.interface = StreamingServerInterface(
            ...
            streaming_interface = crow_agent.interface
            ...
            if stream_steps:
                ...
                async for message in streaming_interface.get_generator():
                    assert (
                        isinstance(message, CrowMessage)
                        or isinstance(message, LegacyCrowMessage)
                ...
                # By default the stream will be messages of type CrowMessage or CrowLegacyMessage
                # If we want to convert these to Message, we can use the attached IDs
                # NOTE: we will need to de-duplicate the Messsage IDs though (since Assistant->Inner+Func_Call)
                # TODO: eventually update the interface to use `Message` and `MessageChunk` (new) inside the deque instead
                return CrowResponse(
                    messages=filtered_stream,
                    stop_reason=CrowStopReason(stop_reason=StopReasonType.end_turn.value),
                    usage=usage,
                )
    ...
    async def send_group_message_to_agent(
        ...
        include_final_message = True
        if not stream_steps and stream_tokens:
            raise CrowInvalidArgumentError("stream_steps must be 'true' if stream_tokens is 'true'", argument_name="stream_steps")
        ...
        crow_multi_agent = load_multi_agent(group=group, agent_state=agent_state, actor=actor)

        llm_config = crow_multi_agent.agent_state.llm_config
        ...
        # Create a new interface per request
        crow_multi_agent.interface = StreamingServerInterface(
            use_assistant_message=use_assistant_message,
            assistant_message_tool_name=assistant_message_tool_name,
            assistant_message_tool_kwarg=assistant_message_tool_kwarg,
            inner_thoughts_in_kwargs=(
        ...
        streaming_interface = crow_multi_agent.interface
        if not isinstance(streaming_interface, StreamingServerInterface):
            raise CrowInvalidArgumentError(f"Agent has wrong type of interface: {type(streaming_interface)}", argument_name="interface")
        streaming_interface.streaming_mode = stream_tokens
        streaming_interface.streaming_chat_completion_mode = chat_completion_mode
        ...
        if stream_steps:
            ...
            async for message in streaming_interface.get_generator():
                assert (
                    isinstance(message, CrowMessage) or isinstance(message, LegacyCrowMessage) or isinstance(message, MessageStreamStatus)
                ), type(message)
                generated_stream.append(message)
                if message == MessageStreamStatus.done:
                    break
