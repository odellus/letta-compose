...
from crow.agents.crow_agent import CrowAgent
from crow.agents.crow_agent_v2 import CrowAgentV2
from crow.agents.crow_agent_v3 import CrowAgentV3
from crow.constants import DEFAULT_MAX_STEPS, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, REDIS_RUN_ID_PREFIX
from crow.data_sources.redis_client import get_redis_client
...
from crow.schemas.agent import AgentRelationships, AgentState, CreateAgent, UpdateAgent
from crow.schemas.agent_file import AgentFileSchema
from crow.schemas.block import BaseBlock, Block, BlockResponse, BlockUpdate
from crow.schemas.enums import AgentType, MessageRole, RunStatus
from crow.schemas.file import AgentFileAttachment, FileMetadataBase, PaginatedAgentFiles
from crow.schemas.group import Group
from crow.schemas.job import CrowRequestConfig
from crow.schemas.crow_message import CrowMessageUnion, CrowMessageUpdateUnion, MessageType
from crow.schemas.crow_message_content import TextContent
from crow.schemas.crow_request import CrowAsyncRequest, CrowRequest, CrowStreamingRequest
from crow.schemas.crow_response import CrowResponse, CrowStreamingResponse
from crow.schemas.crow_stop_reason import StopReasonType
from crow.schemas.mcp_server import ToolExecuteRequest
from crow.schemas.memory import (
...
from crow.schemas.message import Message, MessageCreate, MessageCreateType, MessageSearchRequest, MessageSearchResult
from crow.schemas.passage import Passage
from crow.schemas.run import Run as PydanticRun, RunUpdate
from crow.schemas.source import BaseSource, Source
from crow.schemas.tool import BaseTool, Tool
...
from crow.services.lettuce import LettuceClient
...
from crow.utils import is_1_0_sdk_version, safe_create_shielded_task, safe_create_task, truncate_file_visible_content
from crow.validators import AgentId, BlockId, FileId, MessageId, SourceId, ToolId
...
async def _import_agent(
    ...
    if not import_result.success:
        from crow.errors import AgentFileImportError
...
async def import_agent(
    ...
    override_existing_tools: bool = Form(
        True,
        description="If set to True, existing tools can get their source code overwritten by the uploaded tool definitions. Note that Crow core tools can never be updated externally.",
    ),
...
AgentMessagesResponse = Annotated[
    list[CrowMessageUnion], Field(json_schema_extra={"type": "array", "items": {"$ref": "#/components/schemas/CrowMessageUnion"}})
]
...
@router.patch("/{agent_id}/messages/{message_id}", response_model=CrowMessageUnion, operation_id="modify_message")
async def modify_message(
    agent_id: AgentId,  # backwards compatible. Consider removing for v1
    message_id: MessageId,
    request: CrowMessageUpdateUnion = Body(...),
    server: "SyncServer" = Depends(get_crow_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
    ...
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    return await server.message_manager.update_message_by_crow_message_async(
        message_id=message_id, crow_message_update=request, actor=actor
    )
...
@router.post(
    "/{agent_id}/messages",
    response_model=CrowResponse,
    operation_id="send_message",
    responses={
        200: {
            "description": "Successful response",
            "content": {
                "application/json": {"schema": {"$ref": "#/components/schemas/CrowResponse"}},
...
async def send_message(
    ...
    agent_id: AgentId,
    server: SyncServer = Depends(get_crow_server),
    request: CrowStreamingRequest = Body(...),
    headers: HeaderParams = Depends(get_headers),
) -> StreamingResponse | CrowResponse:
    """
    Process a user message and return the agent's response.
    This endpoint accepts a message from a user and processes it through the agent.

    The response format is controlled by the `streaming` field in the request body:
    - If `streaming=false` (default): Returns a complete CrowResponse with all messages
...
@router.post(
    ...
    response_model=CrowStreamingResponse,
...
async def _process_message_background(
    ...
    try:
        ...
        runs_manager = RunManager()
        from crow.schemas.enums import RunStatus
...
async def _cleanup_background_task_resources(agent_loop: BaseAgentV2 | CrowAgent, result: StreamingResponse | CrowResponse) -> None:
    """
...
async def send_message_async(
    ...
    if use_lettuce:
        ...
        is_v1_message_async_enabled = (
            agent_state.agent_type == AgentType.crow_v1_agent and headers.experimental_params.crow_v1_agent_message_async
        )
        if agent_state.multi_agent_group is None and (agent_state.agent_type != AgentType.crow_v1_agent or is_v1_message_async_enabled):
            lettuce_client = await LettuceClient.create()
...
async def preview_model_request(
    agent_id: AgentId,
    request: Union[CrowRequest, CrowStreamingRequest] = Body(...),
    server: SyncServer = Depends(get_crow_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
...
async def summarize_messages(
    ...
    if agent_eligible and model_compatible:
        agent_loop = CrowAgentV3(agent_state=agent, actor=actor)
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Summarization is not currently supported for this agent configuration. Please contact Crow support.",
        )
