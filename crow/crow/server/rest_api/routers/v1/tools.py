...
from crow.constants import DEFAULT_GENERATE_TOOL_MODEL_HANDLE, MAX_TOOL_NAME_LENGTH
...
from crow.functions.functions import derive_openai_json_schema
from crow.functions.mcp_client.exceptions import MCPTimeoutError
from crow.functions.mcp_client.types import MCPTool, SSEServerConfig, StdioServerConfig, StreamableHTTPServerConfig
from crow.helpers.decorators import deprecated
from crow.llm_api.llm_client import LLMClient
from crow.log import get_logger
from crow.orm.errors import UniqueConstraintViolationError
from crow.orm.mcp_oauth import OAuthSessionStatus
from crow.prompts.gpt_system import get_system_text
from crow.schemas.enums import AgentType, MessageRole, ToolType
from crow.schemas.crow_message import ToolReturnMessage
from crow.schemas.crow_message_content import TextContent
from crow.schemas.mcp import UpdateSSEMCPServer, UpdateStdioMCPServer, UpdateStreamableHTTPMCPServer
from crow.schemas.message import Message
from crow.schemas.pip_requirement import PipRequirement
from crow.schemas.tool import BaseTool, Tool, ToolCreate, ToolRunFromSource, ToolSearchRequest, ToolSearchResult, ToolUpdate
from crow.server.rest_api.dependencies import HeaderParams, get_headers, get_crow_server
from crow.server.rest_api.streaming_response import StreamingResponseWithStatusCode
from crow.server.server import SyncServer
from crow.services.mcp.oauth_utils import MCPOAuthSession, drill_down_exception, oauth_stream_event
from crow.services.mcp.stdio_client import AsyncStdioMCPClient
from crow.services.mcp.types import OauthStreamEvent
from crow.services.summarizer.summarizer import traceback
from crow.settings import tool_settings
from crow.utils import asyncio
from crow.validators import ToolId
...
async def count_tools(
    ...
    return_only_crow_tools: Optional[bool] = Query(False, description="Count only tools with tool_type starting with 'crow_'"),
    exclude_crow_tools: Optional[bool] = Query(False, description="Exclude built-in Crow tools from the count"),
    server: SyncServer = Depends(get_crow_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
    ...
    return await server.tool_manager.count_tools_async(
        ...
        exclude_crow_tools=exclude_crow_tools,
...
async def list_tools(
    ...
    return_only_crow_tools: Optional[bool] = Query(False, description="Return only tools with tool_type starting with 'crow_'"),
    server: SyncServer = Depends(get_crow_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
...
async def list_mcp_tools_by_server(
    ...
    try:
        ...
        raise CrowMCPConnectionError(str(e), server_name=mcp_server_name)
    except HTTPStatusError as e:
        # HTTPStatusError from the MCP server likely means auth issue
        if e.response.status_code == 401:
            raise CrowMCPConnectionError(f"Authentication failed: {e}", server_name=mcp_server_name)
        else:
            raise CrowMCPConnectionError(f"HTTP error from MCP server: {e}", server_name=mcp_server_name)
...
async def add_mcp_tool(
    ...
    """
    Register a new MCP tool as a Crow server by MCP server + tool name
    """
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    ...
    if tool_settings.mcp_read_from_config:
        try:
            ...
            raise CrowMCPTimeoutError(str(e), server_name=mcp_server_name)
...
async def add_mcp_server_to_config(
    ...
    """
    Add a new MCP server to the Crow MCP server config
    """
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    ...
    if tool_settings.mcp_read_from_config:
        ...
        if isinstance(request, StdioServerConfig) and tool_settings.mcp_disable_stdio:
            raise HTTPException(
                status_code=400,
                detail="stdio is not supported in the current environment, please use a self-hosted Crow server in order to add a stdio MCP server",
            )
...
async def test_mcp_server(
    ...
    try:
        ...
        raise CrowMCPConnectionError(str(e), server_name=request.server_name)
    except MCPTimeoutError as e:
        raise CrowMCPTimeoutError(f"MCP server connection timed out: {str(e)}", server_name=request.server_name)
    finally:
        if client:
            try:
...
async def generate_json_schema(
    ...
    if request.source_type == "typescript":
        from crow.functions.typescript_parser import derive_typescript_json_schema
