...
from crow.errors import CrowExpiredError, CrowInvalidArgumentError
from crow.helpers.datetime_helpers import get_utc_time
from crow.schemas.enums import RunStatus
from crow.schemas.crow_message import CrowMessageUnion
from crow.schemas.crow_request import RetrieveStreamRequest
...
from crow.schemas.run import Run
from crow.schemas.run_metrics import RunMetrics
from crow.schemas.step import Step
from crow.server.rest_api.dependencies import HeaderParams, get_headers, get_crow_server
from crow.server.rest_api.redis_stream_manager import redis_sse_stream_generator
                ...
                "text/event-stream": {
                    ...
                    "schema": {
                        "oneOf": [
                            ...
                            {"$ref": "#/components/schemas/CrowPing"},
                            {"$ref": "#/components/schemas/CrowErrorMessage"},
                            {"$ref": "#/components/schemas/CrowStopReason"},
                            {"$ref": "#/components/schemas/CrowUsageStatistics"},
                        ]
                    },
                },
            },
...
async def retrieve_stream_for_run(
    ...
    if not run.background:
        raise CrowInvalidArgumentError("Run was not created in background mode, so it cannot be retrieved.")

    if run.created_at < get_utc_time() - timedelta(hours=3):
        raise CrowExpiredError("Run was created more than 3 hours ago, and is now expired.")
