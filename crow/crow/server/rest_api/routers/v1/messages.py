...
from crow.agents.crow_agent_batch import CrowAgentBatch
from crow.errors import CrowInvalidArgumentError
from crow.log import get_logger
from crow.schemas.job import BatchJob, JobStatus, JobType, JobUpdate
from crow.schemas.crow_message import CrowMessageSearchResult, CrowMessageUnion
from crow.schemas.crow_request import CreateBatch
from crow.schemas.crow_response import CrowBatchMessages
from crow.schemas.message import Message, MessageSearchRequest, MessageSearchResult, SearchAllMessagesRequest
...
@router.post("/search", response_model=List[CrowMessageSearchResult], operation_id="search_all_messages")
async def search_all_messages(
    ...
    return Message.to_crow_search_results_from_list(search_results=results, text_is_assistant_message=True)
...
async def create_batch(
    ...
    if not settings.enable_batch_job_polling:
        logger.warning("Batch job polling is disabled. Enable batch processing by setting CROW_ENABLE_BATCH_JOB_POLLING to True.")
    ...
    try:
        ...
        await batch_runner.step_until_request(batch_requests=payload.requests, crow_batch_job_id=batch_job.id)
...
@router.get(
    ...
    response_model=CrowBatchMessages,
...
async def list_messages_for_batch(
    ...
    messages = await server.batch_manager.get_messages_for_crow_batch_async(
        crow_batch_job_id=batch_id, limit=limit, actor=actor, agent_id=agent_id, ascending=(order == "asc"), before=before, after=after
    )

    return CrowBatchMessages(messages=messages)
...
async def cancel_batch(
    ...
    llm_batch_jobs = await server.batch_manager.list_llm_batch_jobs_async(crow_batch_id=job.id, actor=actor)
