...
from crow.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, CROW_MODEL_ENDPOINT
...
from crow.server.rest_api.utils import get_user_message_from_chat_completions_request, sse_async_generator
...
async def create_chat_completions(
    ...
    crow_agent = server.load_agent(agent_id=agent_id, actor=actor)
    llm_config = crow_agent.agent_state.llm_config
    if llm_config.model_endpoint_type != "openai" or llm_config.model_endpoint == CROW_MODEL_ENDPOINT:
...
async def send_message_to_agent_chat_completions(
    ...
    try:
        ...
        crow_agent.interface = ChatCompletionsStreamingInterface()
        ...
        safe_create_task(
            asyncio.to_thread(
                ...
                actor=actor,
                agent_id=crow_agent.agent_state.id,
