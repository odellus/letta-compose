...
from crow.errors import CrowUnexpectedStreamCancellationError, PendingApprovalError
from crow.log import get_logger
from crow.schemas.enums import RunStatus
from crow.schemas.crow_message import CrowPing
        ...
        while True:
            try:
                ...
                if not stream_exhausted:
                    ...
                    yield f"data: {CrowPing(id=f'ping-{uuid4()}', date=datetime.now(timezone.utc), run_id=run_id).model_dump_json()}\n\n"
                else:
...
class StreamingResponseWithStatusCode(StreamingResponse):
    ...
    async def _protected_stream_response(self, send: Send) -> None:
        ...
        # Handle client timeouts (should throw error to inform user)
            ...
            raise CrowUnexpectedStreamCancellationError("Stream was terminated due to unexpected cancellation from server")
