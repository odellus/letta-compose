...
from crow.errors import ContextWindowExceededError, RateLimitExceededError
...
from crow.otel.tracing import tracer
from crow.schemas.agent import AgentState
from crow.schemas.enums import MessageRole
from crow.schemas.crow_message import ToolReturn as CrowToolReturn
...
from crow.schemas.message import ApprovalCreate, Message, MessageCreate, ToolReturn
...
from crow.system import get_heartbeat, package_function_response
    ...
    try:
        ...
        if usage_task is not None:
            try:
                ...
                if not isinstance(usage, CrowUsageStatistics):
                    err_msg = f"Expected CrowUsageStatistics, got {type(usage)}"
...
def create_approval_response_message_from_input(
    ...
    def maybe_convert_tool_return_message(maybe_tool_return: CrowToolReturn):
        if isinstance(maybe_tool_return, CrowToolReturn):
...
def create_crow_messages_from_llm_response(
    agent_id: str,
    model: str,
...
def create_assistant_messages_from_openai_response(
    ...
    return create_crow_messages_from_llm_response(
        agent_id=agent_id,
        model=model,
...
def convert_in_context_crow_messages_to_openai(in_context_messages: List[Message], exclude_system_messages: bool = False) -> List[dict]:
    """
    Flattens Crow's messages (with system, user, assistant, tool roles, etc.)
