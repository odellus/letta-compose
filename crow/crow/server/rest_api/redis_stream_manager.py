...
from crow.data_sources.redis_client import AsyncRedisClient
from crow.log import get_logger
from crow.schemas.enums import RunStatus
from crow.schemas.crow_message import CrowErrorMessage
from crow.schemas.crow_stop_reason import CrowStopReason, StopReasonType
from crow.schemas.run import RunUpdate
from crow.schemas.user import User
from crow.server.rest_api.streaming_response import RunCancelledException
...
async def create_background_stream_processor(
    ...
    try:
        ...
        if not saw_done and not saw_error:
            ...
            if stop_reason:
                ...
                await writer.write_chunk(
                    run_id=run_id,
                    data=f"data: {CrowStopReason(stop_reason=StopReasonType.error).model_dump_json()}\n\n",
                    is_complete=False,
                )
                await writer.write_chunk(
        ...
        await writer.write_chunk(
            run_id=run_id, data=f"data: {CrowStopReason(stop_reason=stop_reason).model_dump_json()}\n\n", is_complete=False
        )
