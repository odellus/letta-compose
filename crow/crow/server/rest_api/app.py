...
from crow.__init__ import __version__ as crow_version
from crow.agents.exceptions import IncompatibleAgentType
from crow.constants import ADMIN_PREFIX, API_PREFIX, OPENAI_API_PREFIX
from crow.errors import (
    ...
    HandleNotFoundError,
    CrowAgentNotFoundError,
    CrowExpiredError,
    CrowInvalidArgumentError,
    CrowInvalidMCPSchemaError,
    CrowMCPConnectionError,
    CrowMCPTimeoutError,
    CrowServiceUnavailableError,
    CrowToolCreateError,
    CrowToolNameConflictError,
    CrowUnsupportedFileUploadError,
    CrowUserNotFoundError,
...
from crow.helpers.pinecone_utils import get_pinecone_indices, should_use_pinecone, upsert_pinecone_indices
from crow.jobs.scheduler import start_scheduler_with_leader_election
from crow.log import get_logger
from crow.orm.errors import DatabaseTimeoutError, ForeignKeyConstraintViolationError, NoResultFound, UniqueConstraintViolationError
from crow.otel.tracing import get_trace_id
from crow.schemas.crow_message import create_crow_error_message_schema, create_crow_message_union_schema, create_crow_ping_schema
from crow.schemas.crow_message_content import (
    create_crow_assistant_message_content_union_schema,
    create_crow_message_content_union_schema,
    create_crow_user_message_content_union_schema,
)
from crow.server.constants import REST_DEFAULT_PORT
from crow.server.db import db_registry
from crow.server.global_exception_handler import setup_global_exception_handlers

# NOTE(charles): these are extra routes that are not part of v1 but we still need to mount to pass tests
from crow.server.rest_api.auth.index import setup_auth_router  # TODO: probably remove right?
from crow.server.rest_api.interface import StreamingServerInterface
from crow.server.rest_api.middleware import CheckPasswordMiddleware, LoggingMiddleware
from crow.server.rest_api.routers.v1 import ROUTERS as v1_routes
from crow.server.rest_api.routers.v1.organizations import router as organizations_router
from crow.server.rest_api.routers.v1.users import router as users_router  # TODO: decide on admin
from crow.server.rest_api.static_files import mount_static_files
from crow.server.rest_api.utils import SENTRY_ENABLED
from crow.server.server import SyncServer
from crow.settings import settings, telemetry_settings
from crow.validators import PATH_VALIDATORS, PRIMITIVE_ID_PATTERNS
...
def generate_openapi_schema(app: FastAPI):
    ...
    crow_docs = app.openapi_schema.copy()
    crow_docs["paths"] = {k: v for k, v in crow_docs["paths"].items() if not k.startswith("/openai")}
    crow_docs["info"]["title"] = "Crow API"
    crow_docs["components"]["schemas"]["CrowMessageUnion"] = create_crow_message_union_schema()
    crow_docs["components"]["schemas"]["CrowMessageContentUnion"] = create_crow_message_content_union_schema()
    crow_docs["components"]["schemas"]["CrowAssistantMessageContentUnion"] = create_crow_assistant_message_content_union_schema()
    crow_docs["components"]["schemas"]["CrowUserMessageContentUnion"] = create_crow_user_message_content_union_schema()
    crow_docs["components"]["schemas"]["CrowPing"] = create_crow_ping_schema()
    crow_docs["components"]["schemas"]["CrowErrorMessage"] = create_crow_error_message_schema()

    # Update the app's schema with our modified version
    app.openapi_schema = crow_docs
...
random_password = os.getenv("CROW_SERVER_PASSWORD") or generate_password()
...
async def lifespan(app_: FastAPI):
    ...
    try:
        ...
        from crow.monitoring.event_loop_watchdog import start_watchdog
    ...
    try:
        from crow.jobs.scheduler import shutdown_scheduler_and_release_lock
    ...
    if not settings.disable_tracing and settings.sqlalchemy_tracing:
        try:
            from crow.otel.sqlalchemy_instrumentation_integration import teardown_crow_db_instrumentation

            teardown_crow_db_instrumentation()
...
def create_application() -> "FastAPI":
    ...
    # global server
    # server = SyncServer(default_interface_factory=lambda: interface())
    print(f"\n[[ Crow server // v{crow_version} ]]")

    if SENTRY_ENABLED:
        sentry_sdk.init(
            dsn=os.getenv("SENTRY_DSN"),
            environment=os.getenv("CROW_ENVIRONMENT", "undefined"),
    ...
    if telemetry_settings.enable_datadog:
        try:
            ...
            os.environ.setdefault("DD_VERSION", crow_version)
    ...
    app.add_exception_handler(CrowInvalidArgumentError, _error_handler_400)
    app.add_exception_handler(CrowToolCreateError, _error_handler_400)
    app.add_exception_handler(CrowToolNameConflictError, _error_handler_400)
    ...
    app.add_exception_handler(CrowAgentNotFoundError, _error_handler_404_agent)
    app.add_exception_handler(CrowUserNotFoundError, _error_handler_404_user)
    ...
    # 410 Expired errors
    app.add_exception_handler(CrowExpiredError, _error_handler_410)

    # 408 Timeout errors
    app.add_exception_handler(CrowMCPTimeoutError, _error_handler_408)
    app.add_exception_handler(CrowInvalidMCPSchemaError, _error_handler_400)
    ...
    app.add_exception_handler(CrowUnsupportedFileUploadError, _error_handler_415)
    ...
    app.add_exception_handler(CrowServiceUnavailableError, _error_handler_503)
    ...
    @app.exception_handler(CrowMCPConnectionError)
    async def mcp_connection_error_handler(request: Request, exc: CrowMCPConnectionError):
        return JSONResponse(
            status_code=502,
            content={
                "error": {
    ...
    settings.cors_origins.append("https://app.crow.com")

    if (os.getenv("CROW_SERVER_SECURE") == "true") or "--secure" in sys.argv:
    ...
    if otlp_endpoint and not settings.disable_tracing:
        ...
        service_name = f"crow-server-{env_name_suffix.lower()}" if env_name_suffix else "crow-server"
        from crow.otel.metrics import setup_metrics
        from crow.otel.tracing import setup_tracing
        ...
        if settings.sqlalchemy_tracing:
            from crow.otel.sqlalchemy_instrumentation_integration import setup_crow_db_instrumentation

            try:
                setup_crow_db_instrumentation(
...
def start_server(
    ...
    if debug:
        from crow.server.server import logger as server_logger
    ...
    if (os.getenv("LOCAL_HTTPS") == "true") or "--localhttps" in sys.argv:
        ...
        print("▶ View using ADE at: https://app.crow.com/development-servers/local/dashboard\n")
        if importlib.util.find_spec("granian") is not None and settings.use_granian:
            from granian import Granian

            # Experimental Granian engine
            Granian(
                target="crow.server.rest_api.app:app",
                # factory=True,
                interface="asgi",
