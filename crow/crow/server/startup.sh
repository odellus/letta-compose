...
wait_for_postgres() {
    until pg_isready -U "${POSTGRES_USER:-crow}" -h localhost; do
...
if [ -n "$CROW_REDIS_HOST" ]; then
    echo "External Redis configuration detected, using env var CROW_REDIS_HOST=$CROW_REDIS_HOST"
else
    ...
    export CROW_REDIS_HOST="localhost"
    echo "Using internal Redis at: $CROW_REDIS_HOST"
fi

# Check if we're configured for external Postgres
if [ -n "$CROW_PG_URI" ]; then
    echo "External Postgres configuration detected, using env var CROW_PG_URI"
else
    ...
    export CROW_PG_URI="postgresql://${POSTGRES_USER:-crow}:${POSTGRES_PASSWORD:-crow}@localhost:5432/${POSTGRES_DB:-crow}"
    echo "Using internal PostgreSQL at: $CROW_PG_URI"
fi
...
if [ -n "$CROW_SANDBOX_MOUNT_PATH" ]; then
    if ! chmod 777 "$CROW_SANDBOX_MOUNT_PATH"; then
        echo "ERROR: Failed to set permissions for tool execution directory at: $CROW_SANDBOX_MOUNT_PATH"
...
CMD="crow server --host $HOST --port $PORT"
...
echo "Starting Crow Server at http://$HOST:$PORT..."
echo "Executing: $CMD"
exec $CMD
