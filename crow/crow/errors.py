...
class CrowError(Exception):
    """Base class for all Crow related errors."""
...
class PendingApprovalError(CrowError):
...
class CrowToolCreateError(CrowError):
...
class CrowToolNameConflictError(CrowError):
...
class CrowToolNameSchemaMismatchError(CrowToolCreateError):
...
class CrowConfigurationError(CrowError):
...
class CrowAgentNotFoundError(CrowError):
    """Error raised when an agent is not found."""


class CrowUserNotFoundError(CrowError):
    """Error raised when a user is not found."""


class CrowUnsupportedFileUploadError(CrowError):
    """Error raised when an unsupported file upload is attempted."""


class CrowInvalidArgumentError(CrowError):
...
class CrowImageFetchError(CrowError):
...
class CrowMCPError(CrowError):
    """Base error for MCP-related issues."""


class CrowInvalidMCPSchemaError(CrowMCPError):
...
class CrowMCPConnectionError(CrowMCPError):
...
class CrowMCPTimeoutError(CrowMCPError):
...
class CrowServiceUnavailableError(CrowError):
...
class CrowUnexpectedStreamCancellationError(CrowError):
    """Error raised when a streaming request is terminated unexpectedly."""


class CrowExpiredError(CrowError):
...
class BedrockPermissionError(CrowError):
...
class LLMJSONParsingError(CrowError):
...
class LocalLLMConnectionError(CrowError):
...
class ContextWindowExceededError(CrowError):
...
class RateLimitExceededError(CrowError):
...
class CrowMessageError(CrowError):
    """Base error class for handling message-related errors."""

    messages: List[Union["Message", "CrowMessage"]]
    default_error_message: str = "An error occurred with the message."

    def __init__(self, *, messages: List[Union["Message", "CrowMessage"]], explanation: Optional[str] = None) -> None:
        error_msg = self.construct_error_message(messages, self.default_error_message, explanation)
        super().__init__(error_msg)
        self.messages = messages

    @staticmethod
    def construct_error_message(messages: List[Union["Message", "CrowMessage"]], error_msg: str, explanation: Optional[str] = None) -> str:
...
class HandleNotFoundError(CrowError):
...
class RunCancelError(CrowError):
