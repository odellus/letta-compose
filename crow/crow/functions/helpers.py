...
from crow.functions.interface import MultiAgentMessagingInterface
from crow.orm.errors import NoResultFound
from crow.schemas.enums import MessageRole
from crow.schemas.crow_message import AssistantMessage
...
def {mcp_tool_name}(**kwargs):
    raise RuntimeError("Something went wrong - we should never be using the persisted source code for MCP. Please reach out to Crow team")
"""
...
def _parse_crow_response_for_assistant_message(
    target_agent_id: str,
    crow_response: CrowResponse,
) -> Optional[str]:
    messages = []
    for m in crow_response.messages:
...
async def _send_message_to_agent_no_stream(
    ...
    Returns a CrowResponse containing the final messages.
    """
...
async def _async_send_message_with_retries(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            assistant_message = _parse_crow_response_for_assistant_message(target_agent_id, response)
...
def fire_and_forget_send_to_agent(
    ...
    Args:
        ...
        server: The Crow server instance
