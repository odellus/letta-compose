...
from crow.constants import CROW_MODEL_ENDPOINT, REQUEST_HEARTBEAT_PARAM
...
from crow.llm_api.error_utils import is_context_window_overflow_message
from crow.llm_api.helpers import (
...
from crow.local_llm.constants import INNER_THOUGHTS_KWARG, INNER_THOUGHTS_KWARG_DESCRIPTION, INNER_THOUGHTS_KWARG_DESCRIPTION_GO_FIRST
from crow.log import get_logger
from crow.otel.tracing import trace_method
from crow.schemas.agent import AgentType
from crow.schemas.embedding_config import EmbeddingConfig
from crow.schemas.crow_message_content import MessageContentType
...
from crow.schemas.openai.responses_request import ResponsesRequest
from crow.schemas.response_format import JsonSchemaResponseFormat
...
class OpenAIClient(LLMClientBase):
    ...
    def build_request_data_responses(
        ...
        if tools:
            ...
            # TODO relax this?
            if agent_type == AgentType.crow_v1_agent:
                new_tools = []
        ...
        if llm_config.model_endpoint == CROW_MODEL_ENDPOINT:
            if not self.actor:
                # override user id for inference.crow.com
                import uuid
    ...
    def build_request_data(
        ...
        """
        # Shortcut for GPT-5 to use Responses API, but only for crow_v1_agent
        if use_responses_api(llm_config) and agent_type == AgentType.crow_v1_agent:
        ...
        if tools:  # only set tool_choice if tools exist
            if force_tool_call is not None:
                ...
                tool_choice = "required"
            elif self.requires_auto_tool_choice(llm_config) or agent_type == AgentType.crow_v1_agent:
                tool_choice = "auto"
            else:
    ...
    async def convert_response_to_chat_completion(
        ...
        if "object" in response_data and response_data["object"] == "response":
            ...
            if input_details.get("cached_tokens"):
                from crow.schemas.openai.chat_completion_response import UsageStatisticsPromptTokenDetails
            ...
            if output_details.get("reasoning_tokens"):
                from crow.schemas.openai.chat_completion_response import UsageStatisticsCompletionTokenDetails
    ...
    def handle_llm_error(self, e: Exception) -> Exception:
        ...
        # trigger Crow's context window compaction / retry logic.
        #
        # Example message:
