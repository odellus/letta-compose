...
from crow.schemas.enums import SandboxType
from crow.services.summarizer.enums import SummarizationMode

# Define constants here to avoid circular import with crow.log
...
class SummarizerSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="crow_summarizer_", extra="ignore")
    ...
    # The amount of tokens before a system warning about upcoming truncation is sent to Crow
...
# read pg_uri from ~/.crow/pg_uri or set to none, this is to support Crow Desktop
default_pg_uri = None
...
if "--use-file-pg-uri" in sys.argv:
    try:
        with open(Path.home() / ".crow/pg_uri", "r") as f:
            default_pg_uri = f.read()
            print(f"Read pg_uri from ~/.crow/pg_uri: {default_pg_uri}")
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="crow_", extra="ignore")

    crow_dir: Optional[Path] = Field(Path.home() / ".crow", alias="CROW_DIR")
    ...
    # Crow client settings for tool execution
    default_base_url: str = Field(default="http://localhost:8283", description="Default base URL for Crow client in tool execution")
    default_token: Optional[str] = Field(default=None, description="Default token for Crow client in tool execution")

    # enabling crow_agent_v1 architecture
    use_crow_v1_agent: bool = False
    ...
    def crow_pg_uri(self) -> str:
        if self.pg_uri:
            ...
            return "postgresql+pg8000://crow:crow@localhost:5432/crow"

    # add this property to avoid being returned the default
    # reference: https://github.com/crow-ai/crow/issues/1362
    @property
    def crow_pg_uri_no_default(self) -> str:
        if self.pg_uri:
            return self.pg_uri
        elif self.pg_db and self.pg_user and self.pg_password and self.pg_host and self.pg_port:
            return f"postgresql+pg8000://{self.pg_user}:{self.pg_password}@{self.pg_host}:{self.pg_port}/{self.pg_db}"
    ...
    def database_engine(self) -> DatabaseChoice:
        return DatabaseChoice.POSTGRES if self.crow_pg_uri_no_default else DatabaseChoice.SQLITE
...
class TestSettings(Settings):
    model_config = SettingsConfigDict(env_prefix="crow_test_", extra="ignore")

    crow_dir: Path | None = Field(Path.home() / ".crow/test", alias="CROW_TEST_DIR")


class LogSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="crow_logging_", extra="ignore")
    ...
    crow_log_path: Path | None = Field(Path.home() / ".crow" / "logs" / "Crow.log")
...
class TelemetrySettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="crow_telemetry_", extra="ignore")
    ...
    datadog_service_name: str = Field(default="crow-server", description="Service name for Datadog profiling.")
