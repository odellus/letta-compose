...
from crow.otel.tracing import log_attributes, trace_method
from crow.schemas.openai.chat_completion_response import ChatCompletionResponse
from crow.server.rest_api.dependencies import HeaderParams
...
class OpenAIBackcompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "openai.openai_object":
            from crow.openai_backcompat.openai_object import OpenAIObject
...
def validate_function_response(
    ...
    """Check to make sure that a function used by Crow returned a valid response. Truncates to return_char_limit if necessary.
...
def list_agent_config_files(sort="last_modified"):
    ...
    agent_dir = os.path.join(CROW_DIR, "agents")
...
def list_human_files():
    ...
    defaults_dir = os.path.join(crow.__path__[0], "humans", "examples")
    user_dir = os.path.join(CROW_DIR, "humans")

    crow_defaults = os.listdir(defaults_dir)
    crow_defaults = [os.path.join(defaults_dir, f) for f in crow_defaults if f.endswith(".txt")]
    ...
    if os.path.exists(user_dir):
        ...
        user_added = []
    return crow_defaults + user_added


def list_persona_files():
    """List all personas files"""
    defaults_dir = os.path.join(crow.__path__[0], "personas", "examples")
    user_dir = os.path.join(CROW_DIR, "personas")
...
def get_friendly_error_msg(function_name: str, exception_name: str, exception_message: str):
    from crow.constants import MAX_ERROR_MESSAGE_CHAR_LIMIT
...
def log_telemetry(logger: Logger, event: str, **kwargs):
    ...
    """
    from crow.settings import log_settings
...
def safe_create_file_processing_task(coro, file_metadata, server, actor, logger: Logger, label: str = "file processing task"):
    ...
    """
    from crow.schemas.enums import FileProcessingStatus
...
def is_1_0_sdk_version(headers: HeaderParams):
    ...
    2. If user_agent is provided and in the format
        @crow-ai/crow-client/version (node) or
        crow-client/version (python)
    """
    from crow.log import get_logger
    ...
    if len(parts) == 3:
        # Format: @crow-ai/crow-client/version
        if parts[0] != "@crow-ai" or parts[1] != "crow-client":
            return False
    elif len(parts) == 2:
        # Format: crow-client/version
        if parts[0] != "crow-client":
            return False
    else:
        return False
