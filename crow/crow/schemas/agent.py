...
from crow.constants import CORE_MEMORY_LINE_NUMBER_WARNING, DEFAULT_EMBEDDING_CHUNK_SIZE
from crow.errors import AgentExportProcessingError, CrowInvalidArgumentError
from crow.schemas.block import Block, CreateBlock
from crow.schemas.embedding_config import EmbeddingConfig
from crow.schemas.enums import PrimitiveType
from crow.schemas.environment_variables import AgentEnvironmentVariable
from crow.schemas.file import FileStatus
from crow.schemas.group import Group
from crow.schemas.identity import Identity
...
from crow.schemas.memory import Memory
from crow.schemas.message import Message, MessageCreate
from crow.schemas.model import ModelSettingsUnion
from crow.schemas.openai.chat_completion_response import UsageStatistics
from crow.schemas.response_format import ResponseFormatUnion
from crow.schemas.source import Source
from crow.schemas.tool import Tool
from crow.schemas.tool_rule import ToolRule
from crow.services.summarizer.summarizer_config import CompactionSettings
from crow.utils import calculate_file_defaults_based_on_context_window, create_random_username
...
class AgentType(str, Enum):
    ...
    crow_v1_agent = "crow_v1_agent"  # simplification of the memgpt loop, no heartbeats or forced tool calls
...
class AgentState(OrmMetadataBase, validate_assignment=True):
    ...
    kv_cache_friendly: Optional[bool] = Field(
        False,
        description="If set to True, the system prompt will remain immutable between compactions to preserve KV cache for local LLMs. Memory updates will be reflected in tool responses instead of system prompt changes.",
    )
...
class CreateAgent(BaseModel, validate_assignment=True):  #
    ...
    agent_type: AgentType = Field(default_factory=lambda: AgentType.crow_v1_agent, description="The type of agent.")
    ...
    include_base_tools: bool = Field(True, description="If true, attaches the Crow core tools (e.g. core_memory related functions).")
    include_multi_agent_tools: bool = Field(
        False, description="If true, attaches the Crow multi-agent tools (e.g. sending a message to another agent)."
    )
    include_base_tool_rules: Optional[bool] = Field(
        None, description="If true, attaches the Crow base tool rules (e.g. deny all tools not explicitly allowed)."
    )
    ...
    kv_cache_friendly: Optional[bool] = Field(
        None, description="If set to True, the system prompt will remain immutable between compactions to preserve KV cache for local LLMs."
    )
    response_format: Optional[ResponseFormatUnion] = Field(
        None,
    ...
    def validate_model(cls, model: Optional[str]) -> Optional[str]:
        ...
        if "/" not in model:
            raise CrowInvalidArgumentError("The model handle should be in the format provider/model-name", argument_name="model")
