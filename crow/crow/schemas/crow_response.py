...
from crow.schemas.enums import JobStatus, MessageStreamStatus
...
class CrowResponse(BaseModel):
    """
    Response object from an agent interaction, consisting of the new messages generated by the agent and usage statistics.
    The type of the returned messages can be either `Message` or `CrowMessage`, depending on what was specified in the request.

    Attributes:
        messages (List[Union[Message, CrowMessage]]): The messages returned by the agent.
        usage (CrowUsageStatistics): The usage statistics
    """

    messages: List[CrowMessageUnion] = Field(
        ...,
        description="The messages returned by the agent.",
        json_schema_extra={
            "items": {
                "$ref": "#/components/schemas/CrowMessageUnion",
            }
        },
    )
    stop_reason: CrowStopReason = Field(
        ...,
        description="The stop reason from Crow indicating why agent loop stopped execution.",
    )
    usage: CrowUsageStatistics = Field(
        ...,
        ...
        return json_dumps(
            {
                ...
                # Assume `Message` and `CrowMessage` have a `dict()` method
                "usage": self.usage.model_dump(),  # Assume `CrowUsageStatistics` has a `dict()` method
            },
            indent=4,
        )
...
class CrowStreamingResponse(RootModel):
    """
...
class CrowBatchResponse(BaseModel):
    crow_batch_id: str = Field(..., description="A unique identifier for the Crow batch request.")
...
class CrowBatchMessages(BaseModel):
