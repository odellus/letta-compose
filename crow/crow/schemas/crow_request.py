...
from crow.constants import DEFAULT_MAX_STEPS, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG
from crow.schemas.crow_message import MessageType
from crow.schemas.crow_message_content import CrowMessageContentUnion
from crow.schemas.message import MessageCreate, MessageCreateUnion, MessageRole


class CrowRequest(BaseModel):
    messages: Optional[List[MessageCreateUnion]] = Field(None, description="The messages to be sent to the agent.")
    input: Optional[Union[str, List[CrowMessageContentUnion]]] = Field(
    ...
    use_assistant_message: bool = Field(
        default=True,
        description="Whether the server should parse specific tool call arguments (default `send_message`) as `AssistantMessage` objects. Still supported for legacy agent types, but deprecated for crow_v1_agent onward.",
        deprecated=True,
    )
    assistant_message_tool_name: str = Field(
        default=DEFAULT_MESSAGE_TOOL,
        description="The name of the designated message tool. Still supported for legacy agent types, but deprecated for crow_v1_agent onward.",
        deprecated=True,
    )
    assistant_message_tool_kwarg: str = Field(
        default=DEFAULT_MESSAGE_TOOL_KWARG,
        description="The name of the message argument in the designated message tool. Still supported for legacy agent types, but deprecated for crow_v1_agent onward.",
        deprecated=True,
    )
    ...
    def validate_input_or_messages(self):
        ...
        # input can be either a string or List[CrowMessageContentUnion]
        if self.input is not None:
            # Both str and List[CrowMessageContentUnion] are valid content types for MessageCreate
            self.messages = [MessageCreate(role=MessageRole.user, content=self.input, otid=str(uuid.uuid4()))]

        return self


class CrowStreamingRequest(CrowRequest):
