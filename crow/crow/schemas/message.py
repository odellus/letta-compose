...
from crow.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, REQUEST_HEARTBEAT_PARAM, TOOL_CALL_ID_MAX_LEN
from crow.helpers.datetime_helpers import get_utc_time, is_utc_datetime
from crow.helpers.json_helpers import json_dumps
from crow.local_llm.constants import INNER_THOUGHTS_KWARG, INNER_THOUGHTS_KWARG_VERTEX
...
from crow.schemas.crow_message import (
    ...
    CrowMessage,
    CrowMessageReturnUnion,
    CrowMessageSearchResult,
    MessageType,
    ReasoningMessage,
...
from crow.schemas.crow_message_content import (
    ...
    ToolCallContent,
    ToolReturnContent,
    get_crow_message_content_union_str_json_schema,
)
from crow.system import unpack_message
from crow.utils import parse_json, validate_function_response
...
class MessageCreate(MessageCreateBase):
    ...
    content: Union[str, List[CrowMessageContentUnion]] = Field(
        ...,
        description="The content of the message.",
        json_schema_extra=get_crow_message_content_union_str_json_schema(),
    )
...
class ApprovalCreate(MessageCreateBase):
    ...
    approvals: Optional[List[CrowMessageReturnUnion]] = Field(default=None, description="The list of approval responses")
...
class MessageUpdate(BaseModel):
    ...
    content: Optional[Union[str, List[CrowMessageContentUnion]]] = Field(
        default=None,
        description="The content of the message.",
        json_schema_extra=get_crow_message_content_union_str_json_schema(),
    )
...
class Message(BaseMessage):
    """
        Crow's internal representation of a message. Includes methods to convert to/from LLM provider formats.
    ...
    content: Optional[List[CrowMessageContentUnion]] = Field(default=None, description="The content of the message.")
    ...
    @staticmethod
    @trace_method
    def to_crow_messages_from_list(
        ...
        # Convert messages to CrowMessages
        return [
            msg
            for m in messages
            for msg in m.to_crow_messages(
    ...
    @staticmethod
    @trace_method
    def to_crow_search_results_from_list(
        ...
        include_err: Optional[bool] = None,
        text_is_assistant_message: bool = False,
    ) -> List[CrowMessageSearchResult]:
        """Convert MessageSearchResult objects into CrowMessageSearchResult objects.

        This mirrors the behavior of to_crow_messages_from_list, but preserves the
        originating Message.agent_id on each search result variant.
        """

        crow_search_results: List[CrowMessageSearchResult] = []

        for result in search_results:
            message = result.message

            # Convert the underlying Message into CrowMessage variants
            crow_messages = message.to_crow_messages(
                use_assistant_message=use_assistant_message,
                assistant_message_tool_name=assistant_message_tool_name,
                assistant_message_tool_kwarg=assistant_message_tool_kwarg,
                reverse=reverse,
            ...
            for lm in crow_messages:
                if isinstance(lm, SystemMessage):
                    crow_search_results.append(
                        SystemMessageListResult(
                ...
                # Other CrowMessage variants (tool, approval, etc.) are not part of
                # CrowMessageSearchResult and are intentionally skipped here.

        return crow_search_results

    def to_crow_messages(
        self,
        ...
        """Convert message object (in DB format) to the style used by the original Crow API"""
    ...
    def _convert_explicit_tool_returns(self) -> ToolReturnMessage:
        ...
        for tool_return in self.tool_returns:
            ...
            tool_return_obj = CrowToolReturn(
                tool_return=parsed_data["message"],
    ...
    def dict_to_message(
        ...
        content: List[CrowMessageContentUnion] = (
    ...
    def _image_source_to_data_url(image_content: ImageContent) -> Optional[str]:
        ...
        if source.type == ImageSourceType.crow:
            data = getattr(source, "data", None)
            if not data:
                return None
            media_type = getattr(source, "media_type", None) or "image/png"
