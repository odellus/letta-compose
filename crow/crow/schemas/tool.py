...
from crow.constants import (
    ...
    CROW_BUILTIN_TOOL_MODULE_NAME,
    CROW_CORE_TOOL_MODULE_NAME,
    CROW_FILES_TOOL_MODULE_NAME,
    CROW_MULTI_AGENT_TOOL_MODULE_NAME,
    CROW_VOICE_TOOL_MODULE_NAME,
...
from crow.functions.functions import get_json_schema_from_module
from crow.functions.mcp_client.types import MCPTool
from crow.functions.schema_generator import generate_tool_schema_for_mcp
from crow.log import get_logger
from crow.schemas.enums import ToolSourceType, ToolType
from crow.schemas.crow_base import CrowBase
from crow.schemas.npm_requirement import NpmRequirement
from crow.schemas.pip_requirement import PipRequirement

logger = get_logger(__name__)


class BaseTool(CrowBase):
...
class Tool(BaseTool):
    ...
    def refresh_source_code_and_json_schema(self):
        ...
        This method only handles built-in Crow tools.
        """
        ...
        elif self.tool_type in {ToolType.CROW_CORE, ToolType.CROW_MEMORY_CORE, ToolType.CROW_SLEEPTIME_CORE}:
            # If it's crow core tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=CROW_CORE_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.CROW_MULTI_AGENT_CORE}:
            # If it's crow multi-agent tool, we also generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=CROW_MULTI_AGENT_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.CROW_VOICE_SLEEPTIME_CORE}:
            # If it's crow voice tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=CROW_VOICE_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.CROW_BUILTIN}:
            # If it's crow voice tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=CROW_BUILTIN_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.CROW_FILES_CORE}:
            # If it's crow files tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=CROW_FILES_TOOL_MODULE_NAME, function_name=self.name)

        return self


class ToolCreate(CrowBase):
    description: Optional[str] = Field(None, description="The description of the tool.")
    ...
    def from_mcp(cls, mcp_server_name: str, mcp_tool: MCPTool) -> "ToolCreate":
        from crow.functions.helpers import generate_mcp_tool_wrapper
    ...
    def model_dump(self, to_orm: bool = False, **kwargs):
        """
        Override CrowBase.model_dump to explicitly handle 'tags' being None,
...
class ToolRunFromSource(CrowBase):
    source_code: str = Field(..., description="The source code of the function.")
...
class ToolSearchRequest(CrowBase):
    ...
    tool_types: Optional[List[str]] = Field(None, description="Filter by tool types (e.g., 'custom', 'crow_core').")
...
class ToolSearchResult(CrowBase):
