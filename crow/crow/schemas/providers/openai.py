...
from crow.constants import DEFAULT_EMBEDDING_CHUNK_SIZE, LLM_MAX_CONTEXT_WINDOW
...
class OpenAIProvider(Provider):
    ...
    async def check_api_key(self):
        from crow.llm_api.openai import openai_check_valid_api_key  # TODO: DO NOT USE THIS - old code path
    ...
    def _list_llm_models(self, data: list[dict]) -> list[LLMConfig]:
        """
        This handles filtering out LLM Models by provider that meet Crow's requirements.
        """
        configs = []
        for model in data:
    ...
    def _set_model_parameter_tuned_defaults(model_name: str, llm_config: LLMConfig):
        ...
        if "gpt-4o" in model_name or "gpt-4.1-mini" in model_name or model_name == "crow-free":
