...
class BaseMCPServer(CrowBase):
    __id_prefix__ = PrimitiveType.MCP_SERVER.value


# Create Schemas (for POST requests)
class CreateStdioMCPServer(CrowBase):
...
class CreateSSEMCPServer(CrowBase):
...
class CreateStreamableHTTPMCPServer(CrowBase):
...
class UpdateStdioMCPServer(CrowBase):
...
class UpdateSSEMCPServer(CrowBase):
...
class UpdateStreamableHTTPMCPServer(CrowBase):
...
class MCPServerResyncResult(CrowBase):
...
class ToolExecuteRequest(CrowBase):
...
class CreateMCPServerRequest(CrowBase):
...
class UpdateMCPServerRequest(CrowBase):
...
def convert_update_to_internal(request: UpdateMCPServerRequest):
    ...
    from crow.functions.mcp_client.types import MCPServerType as MCPType, StdioServerConfig as StdioCfg
