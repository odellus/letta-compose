...
from crow.agents.helpers import generate_step_id
from crow.constants import (
    CLI_WARNING_PREFIX,
    COMPOSIO_ENTITY_ENV_VAR_KEY,
    ERROR_MESSAGE_PREFIX,
    FIRST_MESSAGE_ATTEMPTS,
    FUNC_FAILED_HEARTBEAT_MESSAGE,
    CROW_CORE_TOOL_MODULE_NAME,
    CROW_MULTI_AGENT_TOOL_MODULE_NAME,
    LLM_MAX_TOKENS,
    READ_ONLY_BLOCK_EDIT_ERROR,
    REQ_HEARTBEAT_MESSAGE,
    SEND_MESSAGE_TOOL_NAME,
)
from crow.errors import ContextWindowExceededError
from crow.functions.ast_parsers import coerce_dict_args_by_annotations, get_function_annotations_from_source
from crow.functions.composio_helpers import execute_composio_action, generate_composio_action_from_func_name
from crow.functions.functions import get_function_from_module
from crow.helpers import ToolRulesSolver
from crow.helpers.composio_helpers import get_composio_api_key
...
from crow.interface import AgentInterface
from crow.llm_api.helpers import calculate_summarizer_cutoff, get_token_counts_for_messages, is_context_overflow_error
from crow.llm_api.llm_api_tools import create
from crow.llm_api.llm_client import LLMClient
from crow.local_llm.constants import INNER_THOUGHTS_KWARG
from crow.local_llm.utils import num_tokens_from_functions, num_tokens_from_messages
from crow.log import get_logger
from crow.memory import summarize_messages
...
from crow.schemas.agent import AgentState, AgentStepResponse, UpdateAgent
from crow.schemas.block import BlockUpdate
from crow.schemas.embedding_config import EmbeddingConfig
from crow.schemas.enums import MessageRole, ProviderType, StepStatus, ToolType
from crow.schemas.crow_message_content import ImageContent, TextContent
from crow.schemas.memory import ContextWindowOverview, Memory
from crow.schemas.message import Message, MessageCreate, ToolReturn
from crow.schemas.openai.chat_completion_response import ChatCompletionResponse, Message as ChatCompletionMessage, UsageStatistics
from crow.schemas.response_format import ResponseFormatType
from crow.schemas.tool import Tool
from crow.schemas.tool_execution_result import ToolExecutionResult
from crow.schemas.tool_rule import TerminalToolRule
...
from crow.services.helpers.agent_manager_helper import check_supports_structured_output
...
from crow.services.telemetry_manager import NoopTelemetryManager, TelemetryManager
from crow.services.tool_executor.tool_execution_sandbox import ToolExecutionSandbox
from crow.services.tool_manager import ToolManager
from crow.settings import model_settings, settings, summarizer_settings
from crow.streaming_interface import StreamingRefreshCLIInterface
from crow.system import get_heartbeat, get_token_limit_warning, package_function_response, package_summarize_message, package_user_message
from crow.utils import count_tokens, get_friendly_error_msg, get_tool_call_id, log_telemetry, parse_json, validate_function_response
...
class Agent(BaseAgent):
    ...
    def _handle_ai_response(
        ...
        if response_message.function_call or (response_message.tool_calls is not None and len(response_message.tool_calls) > 0):
            ...
            target_crow_tool = None
            for t in self.agent_state.tools:
                if t.name == function_name:
                    # This force refreshes the target_crow_tool from the database
                    # We only do this on name match to confirm that the agent state contains a specific tool with the right name
                    target_crow_tool = ToolManager().get_tool_by_name(tool_name=function_name, actor=self.user)
                    break

            if not target_crow_tool:
            ...
            try:
                ...
                log_event(
                    ...
                    attributes={
                        ...
                        "target_crow_tool": target_crow_tool.model_dump(),
                        **{f"function_args.{k}": v for k, v in function_args.items()},
                    },
                )

                tool_execution_result = self.execute_tool_and_persist_state(function_name, function_args, target_crow_tool)
                ...
                return_char_limit = target_crow_tool.return_char_limit
    ...
    def step(
        ...
        **kwargs,
        ...
        return CrowUsageStatistics(**total_usage.model_dump(), step_count=step_count, steps_messages=steps_messages)
    ...
    def execute_tool_and_persist_state(self, function_name: str, function_args: dict, target_crow_tool: Tool) -> ToolExecutionResult:
        """
        ...
        try:
            if target_crow_tool.tool_type == ToolType.CROW_CORE:
                # base tools are allowed to access the `Agent` object and run on the database
                callable_func = get_function_from_module(CROW_CORE_TOOL_MODULE_NAME, function_name)
                function_args["self"] = self  # need to attach self to arg since it's dynamically linked
                function_response = callable_func(**function_args)
            elif target_crow_tool.tool_type == ToolType.CROW_MULTI_AGENT_CORE:
                callable_func = get_function_from_module(CROW_MULTI_AGENT_TOOL_MODULE_NAME, function_name)
                function_args["self"] = self  # need to attach self to arg since it's dynamically linked
                function_response = callable_func(**function_args)
            elif target_crow_tool.tool_type == ToolType.CROW_MEMORY_CORE or target_crow_tool.tool_type == ToolType.CROW_SLEEPTIME_CORE:
                callable_func = get_function_from_module(CROW_CORE_TOOL_MODULE_NAME, function_name)
            ...
            elif target_crow_tool.tool_type == ToolType.EXTERNAL_COMPOSIO:
                action_name = generate_composio_action_from_func_name(target_crow_tool.name)
            ...
            elif target_crow_tool.tool_type == ToolType.EXTERNAL_MCP:
                # Get the server name from the tool tag
                # TODO make a property instead?
                server_name = target_crow_tool.tags[0].split(":")[1]
                ...
                if function_name not in available_tool_names:
                    raise ValueError(
                        f"{function_name} is not available in MCP server {server_name}. Please check your `~/.crow/mcp_config.json` file."
                    )
                ...
                try:
                    ...
                    annotations = get_function_annotations_from_source(target_crow_tool.source_code, function_name)
                ...
                tool_execution_result = ToolExecutionSandbox(function_name, function_args, self.user, tool_object=target_crow_tool).run(
