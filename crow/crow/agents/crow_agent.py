...
from crow.helpers.datetime_helpers import AsyncTimer, get_utc_time, get_utc_timestamp_ns, ns_to_ms
...
from crow.interfaces.anthropic_streaming_interface import AnthropicStreamingInterface
from crow.interfaces.openai_streaming_interface import OpenAIStreamingInterface
from crow.llm_api.llm_client import LLMClient
from crow.llm_api.llm_client_base import LLMClientBase
...
from crow.schemas.enums import JobStatus, ProviderType, StepStatus, ToolType
from crow.schemas.crow_message import MessageType
...
from crow.schemas.message import Message, MessageCreateBase
...
from crow.services.step_manager import NoopStepManager, StepManager
...
class CrowAgent(BaseAgent):
    ...
    async def step_stream_no_tokens(
        ...
        for i in range(max_steps):
            if in_context_messages[-1].role == "approval":
                ...
                if await self._check_run_cancellation():
                    stop_reason = CrowStopReason(stop_reason=StopReasonType.cancelled.value)
                ...
                try:
                    ...
                    try:
                        ...
                        stop_reason = CrowStopReason(stop_reason=StopReasonType.invalid_llm_response.value)
                        raise e
                    ...
                    if not response.choices[0].message.tool_calls:
                        stop_reason = CrowStopReason(stop_reason=StopReasonType.no_tool_call.value)
                    ...
                    crow_messages = [m for m in crow_messages if m.message_type != "approval_response_message"]
                    ...
                    if not stop_reason:
                        stop_reason = CrowStopReason(stop_reason=StopReasonType.error.value)
        ...
        if not agent_state.message_buffer_autoclear:
            await self._rebuild_context_window(
                in_context_messages=current_in_context_messages,
                new_crow_messages=new_in_context_messages,
    ...
    async def _step(
        ...
        request_start_timestamp_ns: int | None = None,
        dry_run: bool = False,
    ) -> Union[tuple[list[Message], list[Message], CrowStopReason | None, CrowUsageStatistics], dict]:
        """
    ...
    async def step_stream(
        ...
        for i in range(max_steps):
            if in_context_messages[-1].role == "approval":
                ...
                # yields tool response as this is handled from Crow and not the response from the LLM provider
                ...
                try:
                    ...
                    try:
                        ...
                        stop_reason = CrowStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
                        raise e
                    ...
                    persisted_messages, should_continue, stop_reason = await self._handle_ai_response(
                        ...
                        pre_computed_assistant_message_id=interface.crow_message_id,
                        step_id=effective_step_id,
                        initial_messages=initial_messages,
                        agent_step_span=agent_step_span,
                        is_final_step=(i == max_steps - 1),
                    ...
                    # Update step with actual usage now that we have it (if step was created)
                    if logged_step:
                        # Build detailed token breakdowns from CrowUsageStatistics
    ...
    async def _handle_llm_error(
        ...
        if isinstance(e, ContextWindowExceededError):
            return await self._rebuild_context_window(
                in_context_messages=in_context_messages, new_crow_messages=new_crow_messages, llm_config=llm_config, force=force
            )
        else:
    ...
    async def summarize_conversation_history(self) -> None:
        ...
        new_in_context_messages, updated = await self.summarizer.summarize(
            in_context_messages=in_context_messages, new_crow_messages=[], force=True
        )
    ...
    async def _create_llm_request_data_async(
        ...
        tools = [
            ...
            in {
                ToolType.CUSTOM,
                ToolType.CROW_CORE,
                ToolType.CROW_MEMORY_CORE,
                ToolType.CROW_MULTI_AGENT_CORE,
                ToolType.CROW_SLEEPTIME_CORE,
                ToolType.CROW_VOICE_SLEEPTIME_CORE,
                ToolType.CROW_BUILTIN,
                ToolType.CROW_FILES_CORE,
