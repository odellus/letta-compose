...
from crow.errors import PendingApprovalError
...
from crow.schemas.message import ApprovalCreate, Message, MessageCreate, MessageCreateBase
...
from crow.server.rest_api.utils import create_approval_response_message_from_input, create_input_messages
from crow.services.message_manager import MessageManager

logger = get_logger(__name__)


def _create_crow_response(
    ...
    """
    Converts the newly created/persisted messages into a CrowResponse.
    """
    ...
    # Convert to Crow messages first
    response_messages = Message.to_crow_messages_from_list(
    ...
    return CrowResponse(messages=response_messages, stop_reason=stop_reason, usage=usage)
...
def merge_and_validate_prefilled_args(tool: "Tool", llm_args: Dict[str, Any], prefilled_args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    from crow.schemas.tool import Tool  # local import to avoid circulars in type hints
