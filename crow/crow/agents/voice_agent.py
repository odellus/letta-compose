...
from crow.agents.voice_sleeptime_agent import VoiceSleeptimeAgent
from crow.constants import DEFAULT_MAX_STEPS, NON_USER_MSG_PREFIX, PRE_EXECUTION_MESSAGE_ARG, REQUEST_HEARTBEAT_PARAM
from crow.helpers.datetime_helpers import get_utc_time
from crow.helpers.tool_execution_helper import add_pre_execution_message, enable_strict_mode, remove_request_heartbeat
from crow.interfaces.openai_chat_completions_streaming_interface import OpenAIChatCompletionsStreamingInterface
...
from crow.schemas.openai.chat_completion_request import (
    AssistantMessage,
...
from crow.server.rest_api.utils import (
    convert_in_context_crow_messages_to_openai,
...
class VoiceAgent(BaseAgent):
    ...
    async def step(self, input_messages: List[MessageCreate], max_steps: int = DEFAULT_MAX_STEPS) -> CrowResponse:
    ...
    async def step_stream(self, input_messages: List[MessageCreate], max_steps: int = DEFAULT_MAX_STEPS) -> AsyncGenerator[str, None]:
        ...
        if agent_state.llm_config.model_endpoint_type == "anthropic":
            ...
            raise ValueError("Crow voice agents are only compatible with OpenAI or Anthropic.")
        ...
        crow_message_db_queue = await create_input_messages(
        ...
        for _ in range(max_steps):
            ...
            openai_messages = convert_in_context_crow_messages_to_openai(in_context_messages, exclude_system_messages=True)
        ...
        await self._rebuild_context_window(summarizer, in_context_messages, crow_message_db_queue)
    ...
    async def _handle_ai_response(
        ...
        crow_message_db_queue: List[Any],
    ) -> bool:
        """
        ...
        if streaming_interface.content_buffer:
            ...
            crow_message_db_queue.extend(assistant_msgs)
        ...
        if streaming_interface.tool_call_happened:
            ...
            crow_message_db_queue.extend(tool_call_messages)
    ...
    async def _rebuild_context_window(
        self, summarizer: Summarizer, in_context_messages: List[Message], crow_message_db_queue: List[Message]
    ) -> None:
        new_crow_messages = await self.message_manager.create_many_messages_async(crow_message_db_queue, actor=self.actor)

        # TODO: Make this more general and configurable, less brittle
        new_in_context_messages, updated = await summarizer.summarize(
            in_context_messages=in_context_messages, new_crow_messages=new_crow_messages
        )
    ...
    def _build_tool_schemas(self, agent_state: AgentState, external_tools_only=True) -> List[Tool]:
        if external_tools_only:
            tools = [
                ...
                if t.tool_type in {ToolType.CUSTOM, ToolType.CROW_FILES_CORE, ToolType.CROW_BUILTIN, ToolType.EXTERNAL_MCP}
            ]
        else:
