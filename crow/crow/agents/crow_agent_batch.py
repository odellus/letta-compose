...
from crow.agents.helpers import _prepare_in_context_messages_async
...
from crow.jobs.types import RequestStatusUpdateInfo, StepStatusUpdateInfo
...
from crow.schemas.enums import AgentStepStatus, JobStatus, MessageStreamStatus, ProviderType, SandboxType, ToolType
from crow.schemas.job import JobUpdate
from crow.schemas.crow_message import LegacyCrowMessage, CrowMessage
...
from crow.schemas.crow_request import CrowBatchRequest
from crow.schemas.crow_response import CrowBatchResponse, CrowResponse
from crow.schemas.llm_batch_job import AgentStepState, LLMBatchItem
from crow.schemas.message import Message, MessageCreate
from crow.schemas.openai.chat_completion_response import ToolCall as OpenAIToolCall
...
from crow.server.rest_api.utils import create_heartbeat_system_message, create_crow_messages_from_llm_response
...
class CrowAgentBatch(BaseAgent):
    def __init__(
        self,
    ...
    async def step_until_request(
        self,
        batch_requests: List[CrowBatchRequest],
        crow_batch_job_id: str,
        ...
        llm_batch_job = await self.batch_manager.create_llm_batch_job_async(
            ...
            crow_batch_job_id=crow_batch_job_id,
        )
        ...
        return CrowBatchResponse(
            crow_batch_id=llm_batch_job.crow_batch_job_id,
    ...
    @trace_method
    async def resume_step_after_request(self, crow_batch_id: str, llm_batch_id: str) -> CrowBatchResponse:
        ...
        if len(next_reqs) == 0:
            await self.job_manager.update_job_by_id_async(
                job_id=crow_batch_id, job_update=JobUpdate(status=JobStatus.completed), actor=self.actor
            )
            return CrowBatchResponse(
                crow_batch_id=llm_batch_job.crow_batch_job_id,
                last_llm_batch_id=llm_batch_job.id,
        ...
        return await self.step_until_request(
            ...
            crow_batch_job_id=crow_batch_id,
    ...
    async def _bulk_rethink_memory_async(self, params: List[ToolExecutionParams]) -> Sequence[tuple[str, ToolExecutionResult]]:
        ...
        for param in params:
            ...
            # TODO: Move 'rethink_memory' to a native Crow tool that we control
    ...
    async def _prepare_next_iteration_async(
        ...
        ctx: _ResumeContext,
        msg_map: Dict[str, List[Message]],
    ) -> Tuple[List[CrowBatchRequest], Dict[str, AgentStepState]]:
        # who continues?
        ...
        batch_reqs: List[CrowBatchRequest] = []
    ...
    def _prepare_tools_per_agent(agent_state: AgentState, tool_rules_solver: ToolRulesSolver) -> List[dict]:
        tools = [t for t in agent_state.tools if t.tool_type in {ToolType.CUSTOM, ToolType.CROW_CORE, ToolType.CROW_MEMORY_CORE}]
    ...
    ) -> AsyncGenerator[Union[CrowMessage, LegacyCrowMessage, MessageStreamStatus], None]:
        raise NotImplementedError
