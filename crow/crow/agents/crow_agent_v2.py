...
from crow.adapters.crow_llm_request_adapter import CrowLLMRequestAdapter
from crow.adapters.crow_llm_stream_adapter import CrowLLMStreamAdapter
from crow.agents.base_agent_v2 import BaseAgentV2
from crow.agents.ephemeral_summary_agent import EphemeralSummaryAgent
from crow.agents.helpers import (
...
from crow.constants import DEFAULT_MAX_STEPS, NON_USER_MSG_PREFIX, REQUEST_HEARTBEAT_PARAM
from crow.errors import ContextWindowExceededError, LLMError
from crow.helpers import ToolRulesSolver
from crow.helpers.datetime_helpers import get_utc_time, get_utc_timestamp_ns, ns_to_ms
from crow.helpers.reasoning_helper import scrub_inner_thoughts_from_messages
from crow.helpers.tool_execution_helper import enable_strict_mode
from crow.llm_api.llm_client import LLMClient
from crow.local_llm.constants import INNER_THOUGHTS_KWARG
from crow.log import get_logger
from crow.otel.tracing import log_event, trace_method, tracer
from crow.prompts.prompt_generator import PromptGenerator
from crow.schemas.agent import AgentState, UpdateAgent
from crow.schemas.enums import AgentType, MessageStreamStatus, RunStatus, StepStatus
from crow.schemas.crow_message import CrowMessage, MessageType
from crow.schemas.crow_message_content import OmittedReasoningContent, ReasoningContent, RedactedReasoningContent, TextContent
...
from crow.schemas.openai.chat_completion_response import (
    FunctionCall,
    ToolCall,
    UsageStatistics,
...
from crow.schemas.step import Step, StepProgression
from crow.schemas.step_metrics import StepMetrics
...
from crow.server.rest_api.utils import (
    create_approval_request_message_from_llm_response,
    create_crow_messages_from_llm_response,
)
...
from crow.services.helpers.tool_parser_helper import runtime_override_tool_json_schema
...
from crow.services.summarizer.enums import SummarizationMode
from crow.services.summarizer.summarizer import Summarizer
...
from crow.settings import model_settings, settings, summarizer_settings
from crow.system import package_function_response
from crow.types import JsonDict
from crow.utils import log_telemetry, safe_create_task, united_diff, validate_function_response


class CrowAgentV2(BaseAgentV2):
    """
    Abstract base class for the Crow agent loop, handling message management,
    ...
    async def build_request(self, input_messages: list[MessageCreate]) -> dict:
        ...
        response = self._step(
            ...
            llm_adapter=CrowLLMRequestAdapter(llm_client=self.llm_client, llm_config=self.agent_state.llm_config),
            dry_run=True,
    ...
    async def step(
        ...
        Returns:
            CrowResponse: Complete response with all messages and metadata
        """
        self._initialize_state()
        ...
        for i in range(max_steps):
            ...
            async for chunk in response:
                response_crow_messages.append(chunk)
        ...
        if not self.agent_state.message_buffer_autoclear:
            await self.summarize_conversation_history(
                ...
                new_crow_messages=self.response_messages,
                total_tokens=self.usage.total_tokens,
                force=False,
            )

        if self.stop_reason is None:
            self.stop_reason = CrowStopReason(stop_reason=StopReasonType.end_turn.value)

        result = CrowResponse(messages=response_crow_messages, stop_reason=self.stop_reason, usage=self.usage)
        if run_id:
    ...
    async def stream(
        ...
        if stream_tokens:
            llm_adapter = CrowLLMStreamAdapter(
                llm_client=self.llm_client,
                llm_config=self.agent_state.llm_config,
                run_id=run_id,
            )
        else:
            llm_adapter = CrowLLMRequestAdapter(
                llm_client=self.llm_client,
                llm_config=self.agent_state.llm_config,
            )
        ...
        try:
            ...
            if self.stop_reason is None:
                ...
                self.stop_reason = CrowStopReason(stop_reason=StopReasonType.max_steps.value)
        ...
        if run_id:
            crow_messages = Message.to_crow_messages_from_list(
            ...
            if not self.stop_reason:
                self.stop_reason = CrowStopReason(stop_reason=StopReasonType.end_turn.value)
            result = CrowResponse(messages=crow_messages, stop_reason=self.stop_reason, usage=self.usage)
            if self.job_update_metadata is None:
                self.job_update_metadata = {}
            self.job_update_metadata["result"] = result.model_dump(mode="json")
    ...
    async def _step(
    ...
    ) -> AsyncGenerator[CrowMessage | dict, None]:
        """
        ...
        Yields:
            CrowMessage or dict: Chunks for streaming mode, or request data for dry_run
        """
        ...
        try:
            ...
            if approval_request and approval_response:
                ...
                if run_id and await self._check_run_cancellation(run_id):
                    self.stop_reason = CrowStopReason(stop_reason=StopReasonType.cancelled.value)
                ...
                for llm_request_attempt in range(summarizer_settings.max_summarizer_retries + 1):
                    try:
                        ...
                        self.stop_reason = CrowStopReason(stop_reason=StopReasonType.invalid_llm_response.value)
                        raise e
                    except LLMError as e:
                        self.stop_reason = CrowStopReason(stop_reason=StopReasonType.llm_api_error.value)
                        raise e
            ...
            if tool_call is None and llm_adapter.tool_call is None:
                self.stop_reason = CrowStopReason(stop_reason=StopReasonType.no_tool_call.value)
            ...
            if llm_adapter.supports_token_streaming():
                if persisted_messages[-1].role != "approval":
                    tool_return = [msg for msg in persisted_messages if msg.role == "tool"][-1].to_crow_messages()[0]
            ...
            if not self.stop_reason:
                self.stop_reason = CrowStopReason(stop_reason=StopReasonType.error.value)
    ...
    def _initialize_state(self):
        ...
        self.usage = CrowUsageStatistics()
        self.last_step_usage: CrowUsageStatistics | None = None  # Per-step usage for Step token details
    ...
    async def _rebuild_memory(
        ...
        # If kv_cache_friendly mode is enabled, skip system prompt rebuilds entirely.
        # Memory updates are tracked via tool responses, and the system prompt
        # will only be rebuilt at compaction time.
        if self.agent_state.kv_cache_friendly:
            self.logger.debug(f"KV cache friendly mode enabled for agent id={self.agent_state.id}, skipping system prompt rebuild")
            return in_context_messages
        ...
        new_system_message_str = PromptGenerator.get_system_message_from_compiled_memory(
            ...
            kv_cache_friendly=agent_state.kv_cache_friendly or False,
        )
    ...
    async def _step_checkpoint_finish(
        ...
        if logged_step:
            ...
            # Build detailed token breakdowns from per-step CrowUsageStatistics
    ...
    def _update_global_usage_stats(self, step_usage_stats: CrowUsageStatistics):
    ...
    async def _handle_ai_response(
    ...
    ) -> tuple[list[Message], bool, CrowStopReason | None]:
        """
        ...
        if is_denial:
            ...
            stop_reason = None
            tool_call_messages = create_crow_messages_from_llm_response(
        ...
        if not is_approval and tool_rules_solver.is_requires_approval_tool(tool_call_name):
            ...
            stop_reason = CrowStopReason(stop_reason=StopReasonType.requires_approval.value)
        else:
    ...
    def _decide_continuation(
    ...
    ) -> tuple[bool, str | None, CrowStopReason | None]:
        continue_stepping = request_heartbeat
        heartbeat_reason: str | None = None
        stop_reason: CrowStopReason | None = None
        ...
        if tool_rule_violated:
            ...
            if tool_rules_solver.is_terminal_tool(tool_call_name):
                if continue_stepping:
                    stop_reason = CrowStopReason(stop_reason=StopReasonType.tool_rule.value)
                continue_stepping = False
        ...
        if is_final_step:
            continue_stepping = False
            stop_reason = CrowStopReason(stop_reason=StopReasonType.max_steps.value)
        else:
    ...
    async def summarize_conversation_history(
        self,
        in_context_messages: list[Message],
        new_crow_messages: list[Message],
        ...
        latest_messages = in_context_messages + new_crow_messages
            ...
            try:
                if force or (total_tokens and total_tokens > self.agent_state.llm_config.context_window):
                    ...
                    new_in_context_messages, updated = await self.summarizer.summarize(
                        in_context_messages=in_context_messages,
                        new_crow_messages=new_crow_messages,
                        force=True,
                        clear=True,
                    )
                else:
                ...
                new_in_context_messages = in_context_messages + new_crow_messages
        else:
            new_in_context_messages = in_context_messages + new_crow_messages
    ...
    async def _log_request(
        ...
        if request_start_timestamp_ns:
            ...
            if request_span:
                request_span.add_event(name="crow_request_ms", attributes={"duration_ms": ns_to_ms(duration_ns)})
