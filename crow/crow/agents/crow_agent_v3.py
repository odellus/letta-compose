...
from crow.adapters.simple_llm_request_adapter import SimpleLLMRequestAdapter
from crow.adapters.simple_llm_stream_adapter import SimpleLLMStreamAdapter
...
from crow.constants import DEFAULT_MAX_STEPS, NON_USER_MSG_PREFIX, REQUEST_HEARTBEAT_PARAM, SUMMARIZATION_TRIGGER_MULTIPLIER
from crow.errors import ContextWindowExceededError, LLMError, SystemPromptTokenExceededError
from crow.helpers import ToolRulesSolver
from crow.helpers.datetime_helpers import get_utc_time, get_utc_timestamp_ns
from crow.helpers.message_helper import convert_message_creates_to_messages
...
from crow.schemas.crow_message import ApprovalReturn, CrowErrorMessage, CrowMessage, MessageType
...
from crow.schemas.message import Message, MessageCreate, ToolReturn
from crow.schemas.openai.chat_completion_response import FunctionCall, ToolCall, ToolCallDenial, UsageStatistics
from crow.schemas.step import StepProgression
...
from crow.services.summarizer.summarizer_all import summarize_all
from crow.services.summarizer.summarizer_config import CompactionSettings
from crow.services.summarizer.summarizer_sliding_window import (
    count_tokens,
    summarize_via_sliding_window,
)
from crow.settings import settings, summarizer_settings
from crow.system import package_function_response, package_summarize_message_no_counts
from crow.utils import log_telemetry, validate_function_response


class CrowAgentV3(CrowAgentV2):
    """
    ...
    * Support Gemini / OpenAI client
    ...
    async def step(
        ...
        response_crow_messages = Message.to_crow_messages_from_list(
            self.response_messages,
        ...
        if include_return_message_types:
            response_crow_messages = [m for m in response_crow_messages if m.message_type in include_return_message_types]
    ...
    async def stream(
        ...
        try:
            if run_id:
                # Filter out CrowStopReason from messages (only valid in CrowStreamingResponse, not CrowResponse)
                filtered_messages = [m for m in response_crow_messages if not isinstance(m, CrowStopReason)]
                result = CrowResponse(messages=filtered_messages, stop_reason=self.stop_reason, usage=self.usage)
                if self.job_update_metadata is None:
                    self.job_update_metadata = {}
                self.job_update_metadata["result"] = result.model_dump(mode="json")
    ...
    async def _check_for_system_prompt_overflow(self, system_message):
        ...
        if system_prompt_token_estimate is not None and system_prompt_token_estimate >= self.agent_state.llm_config.context_window:
            ...
            self.stop_reason = CrowStopReason(stop_reason=StopReasonType.context_window_overflow_in_system_prompt.value)
        ...
        try:
            ...
            if approval_request and approval_response:
                ...
                if not tool_calls and not tool_call_denials and not tool_returns:
                    ...
                    self.should_continue = False
                    self.stop_reason = CrowStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
                    return
                    ...
                    try:
                        ...
                        if isinstance(e, ContextWindowExceededError) and llm_request_attempt < summarizer_settings.max_summarizer_retries:
                            ...
                            try:
                                ...
                                self.stop_reason = CrowStopReason(
                ...
                if tool_calls:
                    ...
                    for tr in response_tool_returns:
                        ...
                        if tr.tool_call_id is None and tr.tool_returns:
                            continue
                        tool_return_crow = tr.to_crow_messages()[0]
                        if include_return_message_types is None or tool_return_crow.message_type in include_return_message_types:
                            yield tool_return_crow
            else:
            ...
            if self.context_token_estimate is not None and self.context_token_estimate > self.agent_state.llm_config.context_window:
                ...
                for message in Message.to_crow_messages(summary_message):
                    yield message
                await self._checkpoint_messages(
                    run_id=run_id, step_id=step_id, new_messages=[summary_message], in_context_messages=messages
                )
    ...
    async def _handle_ai_response(
        ...
        if not tool_calls and not tool_call_denials and not tool_returns:
            ...
            if content is None or len(content) == 0:
                ...
                if uncalled:
                    ...
                    from crow.server.rest_api.utils import create_heartbeat_system_message
            ...
            # Case 1b: No tool call but has content
                ...
                assistant_message = create_crow_messages_from_llm_response(
                    agent_id=self.agent_state.id,
                    model=self.agent_state.llm_config.model,
                    function_name=None,
        ...
        if not is_approval_response:
            ...
            if requested_tool_calls:
                ...
                return messages_to_persist, False, CrowStopReason(stop_reason=StopReasonType.requires_approval.value)
        ...
        persisted_stop_reasons: list[CrowStopReason | None] = []
        ...
        for idx, spec in enumerate(exec_specs):
            ...
            if has_prefill_error:
                cont = False
                hb_reason = None
                sr = CrowStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
            else:
    ...
    def _decide_continuation(
        ...
        if tool_call_name is None:
            ...
            if finish_reason == "length":
                return False, None, CrowStopReason(stop_reason=StopReasonType.max_tokens_exceeded.value)
            return False, None, CrowStopReason(stop_reason=StopReasonType.end_turn.value)
        else:
