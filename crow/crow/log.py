...
from crow.log_context import get_log_context
from crow.settings import log_settings, settings, telemetry_settings
...
class JSONFormatter(logging.Formatter):
    ...
    Usage:
        Enable JSON logging by setting the environment variable:
            CROW_LOGGING_JSON_LOGGING=true
...
class LogContextFilter(logging.Filter):
    ...
    Usage:
        from crow.log_context import set_log_context, update_log_context
...
def _setup_logfile() -> "Path":
    ...
    """
    logfile = Path(settings.crow_dir / "logs" / "Crow.log")
...
def get_logger(name: Optional[str] = None) -> "logging.Logger":
    ...
    """
    parent_logger = logging.getLogger("Crow")
    if name:
