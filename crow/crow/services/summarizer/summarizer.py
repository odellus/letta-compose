...
from crow.prompts import gpt_summarize
from crow.schemas.enums import AgentType, MessageRole, ProviderType
...
from crow.system import package_summarize_message_no_counts
...
class Summarizer:
    ...
    async def summarize(
        ...
        Args:
            ...
            new_crow_messages: The newly added Crow messages (just appended).
        ...
        if self.mode == SummarizationMode.STATIC_MESSAGE_BUFFER:
            result_messages, was_summarized = self._static_buffer_summarization(
                in_context_messages,
                new_crow_messages,
                force=force,
                clear=clear,
            )
            # For kv_cache_friendly mode, rebuild the system message with current memory at compaction
            if was_summarized and self.agent_manager and self.agent_id and self.actor:
                ...
                if agent_state.kv_cache_friendly:
                    ...
                    if new_system_message:
                        result_messages = [new_system_message] + result_messages[1:]
                        logger.info(f"Rebuilt system prompt at compaction for kv_cache_friendly agent {self.agent_id}")
            return result_messages, was_summarized
    ...
    async def _partial_evict_buffer_summarization(
        ...
        """
        all_in_context_messages = in_context_messages + new_crow_messages
        ...
        # For kv_cache_friendly mode, rebuild the system message with current memory
        # This is the natural point to "sync" the system prompt with actual memory state
        system_message = all_in_context_messages[0]
        if agent_state.kv_cache_friendly:
            _, new_system_message, _, _ = await self.agent_manager.rebuild_system_prompt_async(
                agent_id=self.agent_id,
                actor=self.actor,
        ...
        return [system_message, summary_message_obj] + updated_in_context_messages, True
    ...
    def _static_buffer_summarization(
        ...
        Args:
            ...
            new_crow_messages: Newly added messages to append
...
async def simple_summary(
    ...
    async def _run_summarizer_request(req_data: dict, req_messages_obj: list[Message]) -> str:
        ...
        if summarizer_llm_config.model_endpoint_type in [ProviderType.anthropic, ProviderType.bedrock]:
            ...
            from crow.interfaces.anthropic_parallel_tool_call_streaming_interface import (
    ...
    request_data = llm_client.build_request_data(AgentType.crow_v1_agent, input_messages_obj, summarizer_llm_config, tools=[])
    try:
