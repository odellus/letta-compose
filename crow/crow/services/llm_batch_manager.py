...
from crow.jobs.types import BatchPollingResult, ItemUpdateInfo, RequestStatusUpdateInfo, StepStatusUpdateInfo
from crow.log import get_logger
from crow.orm import Message as MessageModel
...
from crow.schemas.enums import AgentStepStatus, JobStatus, ProviderType
from crow.schemas.llm_batch_job import AgentStepState, LLMBatchItem as PydanticLLMBatchItem, LLMBatchJob as PydanticLLMBatchJob
...
class LLMBatchManager:
    ...
    async def list_llm_batch_jobs_async(
        ...
        """
        async with db_registry.async_session() as session:
            query = select(LLMBatchJob).where(LLMBatchJob.crow_batch_job_id == crow_batch_id)
    ...
    @enforce_types
    @trace_method
    async def get_messages_for_crow_batch_async(
        self,
        crow_batch_job_id: str,
        limit: int = 100,
        actor: Optional[PydanticUser] = None,
        ...
        """
        Retrieve messages across all LLM batch jobs associated with a Crow batch job.
        ...
        async with db_registry.async_session() as session:
            ...
            query = (
                ...
                .where(LLMBatchJob.crow_batch_job_id == crow_batch_job_id)
            )
