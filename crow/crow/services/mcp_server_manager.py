...
from crow.functions.mcp_client.types import (
    MCPServerType,
    MCPTool,
    MCPToolHealth,
    SSEServerConfig,
...
from crow.functions.schema_generator import normalize_mcp_schema
...
from crow.orm.mcp_oauth import MCPOAuth, OAuthSessionStatus
from crow.orm.mcp_server import MCPServer as MCPServerModel, MCPTools as MCPToolsModel
from crow.orm.tool import Tool as ToolModel
from crow.schemas.mcp import (
    MCPOAuthSession,
...
from crow.schemas.mcp_server import CreateMCPServerRequest, CreateSSEMCPServer, CreateStdioMCPServer, CreateStreamableHTTPMCPServer
...
from crow.services.mcp.streamable_http_client import AsyncStreamableHTTPMCPClient
from crow.services.tool_manager import ToolManager
from crow.settings import settings, tool_settings
from crow.utils import enforce_types, printd, safe_create_task
...
class MCPServerManager:
    ...
    async def add_tool_from_mcp_server(self, mcp_server_id: str, mcp_tool_name: str, actor: PydanticUser) -> PydanticTool:
        """Add a tool from an MCP server to the Crow tool registry."""
    ...
    async def get_mcp_client(
        ...
        if oauth_provider is None and hasattr(server_config, "server_url"):
            ...
            if oauth_session and await oauth_session.get_access_token_secret().get_plaintext_async():
                ...
                from crow.services.mcp.oauth_utils import create_oauth_provider
    ...
    async def handle_oauth_flow(
        ...
        from crow.services.mcp.oauth_utils import create_oauth_provider, oauth_stream_event
        ...
        CROW_AGENTS_ENDPOINT = os.getenv("CROW_AGENTS_ENDPOINT")
        ...
        elif CROW_AGENTS_ENDPOINT:
            # API and SDK usage should call core server directly
            redirect_uri = f"{CROW_AGENTS_ENDPOINT}/v1/tools/mcp/oauth/callback/{session_id}"
        else:
            logger.error(
                f"No redirect URI found for request and base urls: {http_request.headers if http_request else 'No headers'} {NEXT_PUBLIC_CURRENT_HOST} {CROW_AGENTS_ENDPOINT}"
            )
