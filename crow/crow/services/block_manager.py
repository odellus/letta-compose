...
from crow.orm.block import Block as BlockModel
...
from crow.schemas.block import Block as PydanticBlock, BlockUpdate
from crow.schemas.enums import ActorType, PrimitiveType
...
def validate_block_limit_constraint(update_data: dict, existing_block: BlockModel) -> None:
    ...
    Raises:
        CrowInvalidArgumentError: If validation fails
    """
...
class BlockManager:
    ...
    async def checkpoint_block_async(
        ...
        async with db_registry.async_session() as session:
            ...
            history_entry = BlockHistory(
                ...
                actor_type=ActorType.CROW_AGENT if agent_id else ActorType.CROW_USER,
    ...
    async def undo_checkpoint_block(
        ...
        async with db_registry.async_session() as session:
            ...
            if not block.current_history_entry_id:
                raise CrowInvalidArgumentError(f"Block {block_id} has no history entry - cannot undo.", argument_name="block_id")
    ...
    async def redo_checkpoint_block(
        ...
        async with db_registry.async_session() as session:
            ...
            if not block.current_history_entry_id:
                raise CrowInvalidArgumentError(f"Block {block_id} has no history entry - cannot redo.", argument_name="block_id")
