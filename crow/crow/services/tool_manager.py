...
from crow.constants import (
    ...
    BUILTIN_TOOLS,
    FILES_TOOLS,
    CROW_PARALLEL_SAFE_TOOLS,
    CROW_TOOL_MODULE_NAMES,
    CROW_TOOL_SET,
...
from crow.errors import CrowInvalidArgumentError, CrowToolNameConflictError, CrowToolNameSchemaMismatchError
from crow.functions.functions import derive_openai_json_schema, load_function_set
from crow.helpers.tool_helpers import compute_tool_hash, generate_modal_function_name
from crow.log import get_logger

# TODO: Remove this once we translate all of these to the ORM
from crow.orm.errors import NoResultFound
from crow.orm.tool import Tool as ToolModel
from crow.otel.tracing import trace_method, tracer
from crow.schemas.agent import AgentState
from crow.schemas.enums import PrimitiveType, SandboxType, ToolType
from crow.schemas.tool import Tool as PydanticTool, ToolCreate, ToolUpdate
from crow.schemas.user import User as PydanticUser
from crow.server.db import db_registry
from crow.services.helpers.agent_manager_helper import calculate_multi_agent_tools
from crow.services.mcp.types import SSEServerConfig, StdioServerConfig
from crow.services.tool_schema_generator import generate_schema_for_tool_creation, generate_schema_for_tool_update
from crow.settings import settings, tool_settings
from crow.utils import enforce_types, fire_and_forget, printd
...
def modal_tool_wrapper(tool: PydanticTool, actor: PydanticUser, sandbox_env_vars: dict = None, project_id: str = "default"):
    ...
    packages.append("crow_client>=1.1.1")
    packages.append("crow")  # Base crow without extras
    ...
    secrets_dict = {"CROW_API_KEY": None}
    if sandbox_env_vars:
    ...
    def modal_function(
        tool_name: str, agent_state: Optional[dict], agent_id: Optional[str], env_vars: dict, crow_api_key: Optional[str] = None, **kwargs
    ):
        ...
        if agent_state:
            try:
                from crow.schemas.agent import AgentState as AgentStateModel
        ...
        # TODO: directly instantiate the crow client once we upgrade to 1.0.0+ in core
        # Initialize the Crow client
        # if crow_api_key:
        #     crow_client = Crow(token=crow_api_key, base_url=os.environ.get("CROW_API_URL", "https://api.crow.com"))
        # else:
        #     crow_client = None

        tool_namespace = {
            "__builtins__": __builtins__,  # Include built-in functions
            # "_crow_client": crow_client,  # Make crow_client available
            "os": os,  # Include os module for env vars access
            "agent_id": agent_id,
            "_CROW_API_KEY": crow_api_key,
        ...
        return {
            ...
            "stdout": stdout,
            "stderr": stderr,
            "agent_state": agent_state,  # TODO: deprecate (use crow_client instead)
...
class ToolManager:
    ...
    async def create_or_update_tool_async(
        ...
        if not pydantic_tool.name:
            ...
            if pydantic_tool.name != pydantic_tool.json_schema.get("name"):
                raise CrowToolNameSchemaMismatchError(
    ...
    async def _atomic_upsert_tool_postgresql(
        ...
        from crow.helpers.tpuf_client import should_use_tpuf_for_tools
    ...
    async def list_tools_async(
        ...
        return_only_crow_tools: bool = False,
        ...
        tools = await self._list_tools_async(
            ...
            tool_ids=tool_ids,
            search=search,
            return_only_crow_tools=return_only_crow_tools,
        ...
        if not after and upsert_base_tools:
            ...
            base_tool_names = CROW_TOOL_SET - set(LOCAL_ONLY_MULTI_AGENT_TOOLS) if settings.environment == "prod" else CROW_TOOL_SET
    ...
    async def _list_tools_async(
        ...
        async with db_registry.async_session() as session:
            ...
            # Apply return_only_crow_tools filter at database level
            if return_only_crow_tools:
                query = query.where(ToolModel.tool_type.like("crow_%"))
    ...
    async def count_tools_async(
        ...
        async with db_registry.async_session() as session:
            ...
            # Handle exclude_crow_tools logic (if True, exclude Crow tools)
            if exclude_crow_tools:
                # Exclude tools that are in the CROW_TOOL_SET
                crow_tool_names = list(CROW_TOOL_SET)
                query = query.where(~ToolModel.name.in_(crow_tool_names))
    ...
    async def size_async(
        ...
        async with db_registry.async_session() as session:
            ...
            return await ToolModel.size_async(db_session=session, actor=actor, name=CROW_TOOL_SET)
    ...
    async def update_tool_by_id_async(
        ...
        async with db_registry.async_session() as session:
            ...
            if needs_name_conflict_check:
                ...
                if name_conflict:
                    raise CrowToolNameConflictError(tool_name=new_name)
    ...
    async def upsert_base_tools_async(
        ...
        functions_to_schema = {}
        for module_name in CROW_TOOL_MODULE_NAMES:
            try:
        ...
        for name, schema in functions_to_schema.items():
            if name not in CROW_TOOL_SET:
                continue

            if name in BASE_TOOLS:
                tool_type = ToolType.CROW_CORE
            elif name in BASE_MEMORY_TOOLS:
                tool_type = ToolType.CROW_MEMORY_CORE
            elif name in BASE_SLEEPTIME_TOOLS:
                tool_type = ToolType.CROW_SLEEPTIME_CORE
            elif name in calculate_multi_agent_tools():
                tool_type = ToolType.CROW_MULTI_AGENT_CORE
            elif name in BASE_VOICE_SLEEPTIME_TOOLS or name in BASE_VOICE_SLEEPTIME_CHAT_TOOLS:
                tool_type = ToolType.CROW_VOICE_SLEEPTIME_CORE
            elif name in BUILTIN_TOOLS:
                tool_type = ToolType.CROW_BUILTIN
            elif name in FILES_TOOLS:
                tool_type = ToolType.CROW_FILES_CORE
            else:
            ...
            parallel_safe = name in CROW_PARALLEL_SAFE_TOOLS
    ...
    async def create_or_update_modal_app(self, tool: PydanticTool, actor: PydanticUser):
        ...
        with modal.enable_output(show_progress=False):
            try:
                ...
                raise CrowInvalidArgumentError(f"Failed to deploy tool {tool.id} with name {tool.name} to Modal: {e}")
    ...
    async def search_tools_async(
        ...
        """
        from crow.helpers.tpuf_client import TurbopufferClient, should_use_tpuf_for_tools
