...
from crow.constants import CONVERSATION_SEARCH_TOOL_NAME, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG
...
from crow.schemas.enums import MessageRole, PrimitiveType
from crow.schemas.crow_message import CrowMessageUpdateUnion
from crow.schemas.crow_message_content import ImageSourceType, CrowImage, MessageContentType, TextContent
from crow.schemas.message import Message as PydanticMessage, MessageSearchResult, MessageUpdate
...
from crow.utils import enforce_types, fire_and_forget
...
class MessageManager:
    ...
    def _combine_assistant_tool_messages(self, messages: List[PydanticMessage]) -> List[PydanticMessage]:
        ...
        """
        from crow.constants import DEFAULT_MESSAGE_TOOL
    ...
    async def create_many_messages_async(
        ...
        from crow.helpers.tpuf_client import should_use_tpuf_for_messages
    ...
    @enforce_types
    @trace_method
    async def update_message_by_crow_message_async(
        self, message_id: str, crow_message_update: CrowMessageUpdateUnion, actor: PydanticUser
    ) -> PydanticMessage:
        """
        Updated the underlying messages table giving an update specified to the user-facing CrowMessage
        """
        message = await self.get_message_by_id_async(message_id=message_id, actor=actor)
        if crow_message_update.message_type == "assistant_message":
            ...
            original_args["message"] = crow_message_update.content  # override the assistant message
        ...
        elif crow_message_update.message_type == "reasoning_message":
            update_message = MessageUpdate(content=crow_message_update.reasoning)
        elif crow_message_update.message_type == "user_message" or crow_message_update.message_type == "system_message":
            update_message = MessageUpdate(content=crow_message_update.content)
        else:
            raise ValueError(f"Unsupported message type for modification: {crow_message_update.message_type}")

        message = await self.update_message_by_id_async(message_id=message_id, message_update=update_message, actor=actor)

        # convert back to CrowMessage
        for crow_msg in message.to_crow_messages(use_assistant_message=True):
            if crow_msg.message_type == crow_message_update.message_type:
                return crow_msg

        # raise error if message type got modified
        raise ValueError(f"Message type got modified: {crow_message_update.message_type}")
    ...
    async def delete_message_by_id_async(self, message_id: str, actor: PydanticUser, strict_mode: bool = False) -> bool:
        ...
        from crow.helpers.tpuf_client import TurbopufferClient, should_use_tpuf_for_messages
