...
from crow.functions.helpers import generate_model_from_args_json_schema
...
from crow.services.helpers.tool_execution_helper import add_imports_and_pydantic_schemas_for_args
from crow.services.helpers.tool_parser_helper import convert_param_to_str_value, parse_function_arguments
from crow.services.sandbox_config_manager import SandboxConfigManager
from crow.services.tool_manager import ToolManager
from crow.types import JsonDict, JsonValue
...
class AsyncToolSandboxBase(ABC):
    ...
    async def _init_async(self):
        ...
        if not self._initialized:
            ...
            # Always inject Crow client (available as `client` variable in sandbox)
            self.inject_crow_client = True
    ...
    async def generate_execution_script(self, agent_state: Optional[AgentState], wrap_print_with_markers: bool = False) -> str:
        ...
        code = self._render_sandbox_code(
            ...
            inject_crow_client=self.inject_crow_client,
    ...
    def _render_sandbox_code(
        ...
        *,
        ...
        if inject_agent_state:
            lines.extend(["import crow", "from crow import *"])  # noqa: F401

        # Import Crow client if available (wrapped in try/except for sandboxes without crow_client installed)
        if inject_crow_client:
            lines.extend(
                [
                    "try:",
        ...
        # Initialize Crow client if needed (client is always available as a variable, may be None)
        if inject_crow_client:
            lines.extend(
                [
                    "# Initialize Crow client for tool execution",
                    "import os",
                    "client = None",
                    "if Crow is not None and os.getenv('CROW_API_KEY'):",
                    "    # Check crow_client version to use correct parameter name",
                    "    from packaging import version as pkg_version",
                    "    import crow_client as lc_module",
                    "    lc_version = pkg_version.parse(lc_module.__version__)",
                    "    if lc_version < pkg_version.parse('1.0.0'):",
                    "        client = Crow(",
                    "            token=os.getenv('CROW_API_KEY')",
                    "        )",
                    "    else:",
                    "        client = Crow(",
    ...
    async def _gather_env_vars(self, agent_state: AgentState | None, additional_env_vars: dict[str, str], sbx_id: str, is_local: bool):
        ...
        if self.agent_id:
            env["CROW_AGENT_ID"] = self.agent_id
        if self.project_id:
            env["CROW_PROJECT_ID"] = self.project_id
        env["CROW_TOOL_ID"] = self.tool_id
