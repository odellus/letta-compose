...
from crow.log_context import update_log_context
from crow.orm.agent import Agent as AgentModel
from crow.orm.errors import NoResultFound
from crow.orm.message import Message as MessageModel
from crow.orm.run import Run as RunModel
from crow.orm.run_metrics import RunMetrics as RunMetricsModel
from crow.orm.sqlalchemy_base import AccessType
from crow.orm.step import Step as StepModel
from crow.otel.tracing import log_event, trace_method
from crow.schemas.enums import AgentType, ComparisonOperator, MessageRole, PrimitiveType, RunStatus
from crow.schemas.job import CrowRequestConfig
from crow.schemas.crow_message import CrowMessage, CrowMessageUnion
...
from crow.schemas.message import Message as PydanticMessage
from crow.schemas.run import Run as PydanticRun, RunUpdate
from crow.schemas.run_metrics import RunMetrics as PydanticRunMetrics
from crow.schemas.step import Step as PydanticStep
from crow.schemas.usage import CrowUsageStatistics, normalize_cache_tokens, normalize_reasoning_tokens
...
from crow.services.helpers.agent_manager_helper import validate_agent_exists_async
from crow.services.message_manager import MessageManager
from crow.services.step_manager import StepManager
from crow.utils import enforce_types
...
class RunManager:
    ...
    async def list_runs(
        ...
        async with db_registry.async_session() as session:
            ...
            if order_by == "duration":
                ...
                from crow.services.helpers.run_manager_helper import _apply_pagination_async
    ...
    async def update_run_by_id_async(
        ...
        if is_terminal_update and update.stop_reason:
            try:
                from crow.schemas.agent import UpdateAgent
        ...
        if needs_callback:
            if refresh_result_messages:
                result = CrowResponse(
                    ...
                    stop_reason=CrowStopReason(stop_reason=pydantic_run.stop_reason),
    ...
    @enforce_types
    @raise_on_invalid_id(param_name="run_id", expected_prefix=PrimitiveType.RUN)
    async def get_run_usage(self, run_id: str, actor: PydanticUser) -> CrowUsageStatistics:
        ...
        total_usage = CrowUsageStatistics()
        for step in steps:
    ...
    async def get_run_messages(
        ...
        text_is_assistant_message = agent.agent_type == AgentType.crow_v1_agent
        ...
        crow_messages = PydanticMessage.to_crow_messages_from_list(
        ...
        if request_config and request_config.include_return_message_types:
            ...
            crow_messages = [msg for msg in crow_messages if msg.message_type in include_return_message_types_set]

        return crow_messages

    @enforce_types
    @raise_on_invalid_id(param_name="run_id", expected_prefix=PrimitiveType.RUN)
    async def get_run_request_config(self, run_id: str, actor: PydanticUser) -> Optional[CrowRequestConfig]:
        """Get the crow request config from a run."""
        async with db_registry.async_session() as session:
            run = await RunModel.read_async(db_session=session, identifier=run_id, actor=actor, access_type=AccessType.ORGANIZATION)
            if not run:
                raise NoResultFound(f"Run with id {run_id} not found")
    ...
    async def cancel_run(self, actor: PydanticUser, agent_id: Optional[str] = None, run_id: Optional[str] = None) -> None:
        ...
        if was_pending_approval:
            ...
            if approval_request_message.tool_calls:
                from crow.constants import TOOL_CALL_DENIAL_ON_CANCEL
                from crow.schemas.crow_message import ApprovalReturn
                from crow.schemas.message import ApprovalCreate
