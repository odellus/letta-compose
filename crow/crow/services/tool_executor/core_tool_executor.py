...
from crow.schemas.enums import MessageRole, TagMatchMode
...
class CrowCoreToolExecutor(ToolExecutor):
    """Executor for CROW core tools with direct implementation of functions."""
    ...
    async def conversation_search(
        ...
        try:
            ...
            from crow.constants import CONVERSATION_SEARCH_TOOL_NAME
    ...
    async def core_memory_append(self, agent_state: AgentState, actor: User, label: str, content: str) -> str:
        ...
        # Return diff-style response so agent can track changes
        return f"Memory block '{label}' updated.\nOperation: append\nContent added: {content}\nCharacters: {len(new_value)}/{block.limit}"
    ...
    async def core_memory_replace(
        ...
        # Return diff-style response so agent can track changes
        return (
            f"Memory block '{label}' updated.\n"
            f"Operation: replace\n"
            f"Old content: {old_content}\n"
            f"New content: {new_content}\n"
            f"Characters: {len(new_value)}/{block.limit}"
        )

    async def memory_read(self, agent_state: AgentState, actor: User, label: Optional[str] = None) -> str:
        """Read current memory state from database.

        The memory shown in the system prompt is a snapshot from session start.
        Use this tool to see the current state of memory blocks after updates.

        Args:
            label: Specific block to read, or None for all blocks

        Returns:
            Current block content(s) from memory
        """
        if label:
            block = agent_state.memory.get_block(label)
            if block is None:
                raise ValueError(f"Memory block '{label}' not found")
            return f"Memory block '{label}':\nCharacters: {len(block.value)}/{block.limit}\n---\n{block.value}"
        else:
            # Return all blocks
            result_parts = []
            for lbl in agent_state.memory.list_block_labels():
                block = agent_state.memory.get_block(lbl)
                result_parts.append(f"[{lbl}] ({len(block.value)}/{block.limit} chars):\n{block.value}")
            return "\n\n".join(result_parts)
