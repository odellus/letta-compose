...
class CrowMultiAgentToolExecutor(ToolExecutor):
    """Executor for CROW multi-agent core tools."""
    ...
    async def send_message_to_agents_matching_tags_async(
        ...
        augmented_message = (
            "[Incoming message from external Crow agent - to reply to this message, "
    ...
    async def _process_agent(self, agent_state: AgentState, message: str, actor: User) -> Dict[str, Any]:
        ...
        try:
            ...
            crow_agent = CrowAgentV3(
                agent_state=agent_state,
                actor=self.actor,
            )

            crow_response = await crow_agent.step(
                [MessageCreate(role=MessageRole.system, content=[TextContent(text=message)])], run_id=run.id
            )
            messages = crow_response.messages
