...
from crow.orm.sandbox_config import AgentEnvironmentVariable, AgentEnvironmentVariable as AgentEnvironmentVariableModel
from crow.orm.sqlalchemy_base import AccessType
from crow.otel.tracing import trace_method
from crow.prompts.prompt_generator import PromptGenerator
from crow.schemas.agent import (
    AgentRelationships,
...
from crow.schemas.block import DEFAULT_BLOCKS, Block as PydanticBlock, BlockUpdate
from crow.schemas.embedding_config import EmbeddingConfig
from crow.schemas.enums import AgentType, PrimitiveType, ProviderType, TagMatchMode, ToolType, VectorDBProvider
from crow.schemas.file import FileMetadata as PydanticFileMetadata
from crow.schemas.group import Group as PydanticGroup, ManagerType
from crow.schemas.crow_stop_reason import StopReasonType
from crow.schemas.llm_config import LLMConfig
from crow.schemas.memory import ContextWindowOverview, Memory
from crow.schemas.message import Message, Message as PydanticMessage, MessageCreate, MessageUpdate
from crow.schemas.passage import Passage as PydanticPassage
from crow.schemas.secret import Secret
from crow.schemas.source import Source as PydanticSource
from crow.schemas.tool import Tool as PydanticTool
from crow.schemas.tool_rule import ContinueToolRule, RequiresApprovalToolRule, TerminalToolRule
from crow.schemas.user import User as PydanticUser
from crow.serialize_schemas import MarshmallowAgentSchema
from crow.serialize_schemas.marshmallow_message import SerializedMessageSchema
from crow.serialize_schemas.marshmallow_tool import SerializedToolSchema
from crow.serialize_schemas.pydantic_agent_schema import AgentSchema
from crow.server.db import db_registry
from crow.services.archive_manager import ArchiveManager
from crow.services.block_manager import BlockManager, validate_block_limit_constraint
from crow.services.context_window_calculator.context_window_calculator import ContextWindowCalculator
from crow.services.context_window_calculator.token_counter import create_token_counter
from crow.services.file_processor.chunker.line_chunker import LineChunker
from crow.services.files_agents_manager import FileAgentManager
from crow.services.helpers.agent_manager_helper import (
    _apply_filters,
...
from crow.settings import DatabaseChoice, model_settings, settings
from crow.utils import calculate_file_defaults_based_on_context_window, enforce_types, united_diff
from crow.validators import raise_on_invalid_id
...
class AgentManager:
    ...
    async def create_agent_async(
        ...
        if agent_create.agent_type == AgentType.crow_v1_agent:
        ...
        if agent_create.include_base_tools:
            ...
            elif agent_create.agent_type == AgentType.crow_v1_agent:
                tool_names |= calculate_base_tools(is_v2=True)
        ...
        async with db_registry.async_session() as session:
            async with session.begin():
                ...
                new_agent = AgentModel(
                    ...
                    enable_sleeptime=agent_create.enable_sleeptime,
                    kv_cache_friendly=agent_create.kv_cache_friendly,
    ...
    async def update_agent_async(
        ...
        async with db_registry.async_session() as session, session.begin():
            ...
            scalar_updates = {
                ...
                "kv_cache_friendly": agent_update.kv_cache_friendly,
    ...
    async def get_agent_archive_ids_async(self, agent_id: str, actor: PydanticUser) -> List[str]:
        ...
        from crow.orm import ArchivesAgents
    ...
    async def validate_agent_exists_async(self, agent_id: str, actor: PydanticUser) -> None:
        ...
        Raises:
            CrowAgentNotFoundError: If agent doesn't exist or user doesn't have access
        """
        async with db_registry.async_session() as session:
    ...
    async def reset_messages_async(
        ...
        async with db_registry.async_session() as session:
            ...
            if add_default_initial_messages or needs_agent_state or agent.kv_cache_friendly:
                agent_state = await agent.to_pydantic_async(include_relationships=["sources"] if add_default_initial_messages else None)
            else:
                agent_state = None

        # For kv_cache_friendly mode, rebuild system prompt with current memory on reset
        # This is a natural sync point (like compaction)
        if agent_state and agent_state.kv_cache_friendly:
            await self.rebuild_system_prompt_async(agent_id=agent_id, actor=actor, force=True)
            logger.info(f"Rebuilt system prompt on message reset for kv_cache_friendly agent {agent_id}")
    ...
    async def update_memory_if_changed_async(self, agent_id: str, new_memory: Memory, actor: PydanticUser) -> PydanticAgentState:
        ...
        if new_memory_str not in system_message.content[0].text:
            ...
            # Skip system prompt rebuild if kv_cache_friendly mode is enabled.
            # In that mode, memory updates are tracked via tool responses and
            # the system prompt is only rebuilt at compaction time.
        ...
        if embed_query and query_text and embedding_config:
            ...
            if target_archive_id:
                ...
                if archive.vector_db_provider == VectorDBProvider.TPUF:
                    from crow.helpers.tpuf_client import TurbopufferClient
                    from crow.llm_api.llm_client import LLMClient
    ...
    async def attach_tool_async(self, agent_id: str, tool_id: str, actor: PydanticUser) -> None:
        ...
        async with db_registry.async_session() as session:
            ...
            if settings.crow_pg_uri_no_default:
                from sqlalchemy.dialects.postgresql import insert as pg_insert
    ...
    async def attach_missing_files_tools_async(self, agent_state: PydanticAgentState, actor: PydanticUser) -> PydanticAgentState:
        ...
        attached_file_tool_names = {tool.name for tool in agent_state.tools if tool.tool_type == ToolType.CROW_FILES_CORE}
        ...
        async with db_registry.async_session() as session:
            query = select(ToolModel).where(
                ...
                ToolModel.tool_type == ToolType.CROW_FILES_CORE,
            )
            result = await session.execute(query)
    ...
    async def detach_all_files_tools_async(self, agent_state: PydanticAgentState, actor: PydanticUser) -> PydanticAgentState:
        ...
        file_tool_ids = [tool.id for tool in agent_state.tools if tool.tool_type == ToolType.CROW_FILES_CORE]
        ...
        # create a shallow copy with updated tools list to avoid modifying input
        agent_state_dict = agent_state.model_dump()
        agent_state_dict["tools"] = [tool for tool in agent_state.tools if tool.tool_type != ToolType.CROW_FILES_CORE]
