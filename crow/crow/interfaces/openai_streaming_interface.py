...
from crow.llm_api.openai_client import is_openai_reasoning_model
...
from crow.schemas.openai.chat_completion_response import FunctionCall, ToolCall
from crow.server.rest_api.json_parser import OptimisticJSONParser
from crow.server.rest_api.utils import decrement_message_uuid
from crow.services.context_window_calculator.token_counter import create_token_counter
from crow.streaming_utils import (
...
class OpenAIStreamingInterface:
    ...
    def __init__(
        ...
        self.crow_message_id = Message.generate_id()
    ...
    async def process(
    ...
    ) -> AsyncGenerator[CrowMessage | CrowStopReason, None]:
        """
        ...
        try:
            ...
            yield CrowStopReason(stop_reason=StopReasonType.error)
            raise e
        finally:
            logger.info(
            ...
            if message_delta.tool_calls is not None and len(message_delta.tool_calls) > 0:
                ...
                if not self.emitted_hidden_reasoning and is_openai_reasoning_model(self.model) and not self.put_inner_thoughts_in_kwarg:
                    ...
                    hidden_message = HiddenReasoningMessage(
                        id=self.crow_message_id,
                        date=datetime.now(timezone.utc),
                        state="omitted",
                        hidden_reasoning=None,
                        otid=Message.generate_otid_from_id(self.crow_message_id, message_index),
                        run_id=self.run_id,
                            ...
                            if self.use_assistant_message and self._get_function_name_buffer() == self.assistant_message_tool_name:
                                ...
                                if self.tool_call_name in self.requires_approval_tools:
                                    tool_call_msg = ApprovalRequestMessage(
                                        id=decrement_message_uuid(self.crow_message_id),
                                        date=datetime.now(timezone.utc),
                                        ...
                                        otid=Message.generate_otid_from_id(decrement_message_uuid(self.crow_message_id), -1),
                                        run_id=self.run_id,
                                        step_id=self.step_id,
                                    )
                                else:
    ...
    def get_content(self) -> list[TextContent | SummarizedReasoningContent]:
        ...
        for response in self.final_response.output:
            if isinstance(response, ResponseReasoningItem):
                ...
                crow_summary = [SummarizedReasoningContentPart(index=i, text=part.text) for i, part in enumerate(response.summary)]
                content.append(
                    SummarizedReasoningContent(
                        id=response.id,
                        summary=crow_summary,
