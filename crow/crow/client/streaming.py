...
from crow.errors import LLMError
from crow.log import get_logger
from crow.schemas.enums import MessageStreamStatus
from crow.schemas.crow_message import AssistantMessage, HiddenReasoningMessage, ReasoningMessage, ToolCallMessage, ToolReturnMessage
from crow.schemas.crow_response import CrowStreamingResponse
from crow.schemas.usage import CrowUsageStatistics

logger = get_logger(__name__)


def _sse_post(url: str, data: dict, headers: dict) -> Generator[Union[CrowStreamingResponse, ChatCompletionChunk], None, None]:
    """
                ...
                for sse in event_source.iter_sse():
                    if sse.data in {status.value for status in MessageStreamStatus}:
                        ...
                        if "reasoning" in chunk_data:
                            ...
                            yield CrowUsageStatistics(**chunk_data)
